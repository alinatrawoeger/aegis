...
var GLOBAL_VAR_NAME = '&ZQZ=s&';
var GLOBAL_VAR_CHECKSUM = 'CSUM';
(function () {
    ...
    $axure.utils.makeBindable = function (obj, events) {
        if (obj.registeredBindings != null)
            return;
        // copy the events
    ...
    $axure.utils.parseGlobalVars = function (query, setAction) {
        ...
        for (var i = 0; i < vars.length; i++) {
            ...
            var varValue = pair[1];
            if (varName) {
                if (varName == GLOBAL_VAR_CHECKSUM)
                    csum = true;
                else
                    setAction(varName, decodeURIComponent(varValue), true);
            }
        }
    ...
    var matrixBase = {
        mul: function (val) {
            if (val.x !== undefined) {
                return $axure.utils.Vector2D(this.m11 * val.x + this.m12 * val.y + this.tx, this.m21 * val.x + this.m22 * val.y + this.ty);
            }
            else if (val.m11) {
                return $axure.utils.Matrix2D(this.m11 * val.m11 + this.m12 * val.m21, this.m11 * val.m12 + this.m12 * val.m22, this.m21 * val.m11 + this.m22 * val.m21, this.m21 * val.m12 + this.m22 * val.m22, val.tx + this.tx * val.m11 + this.ty * val.m21, val.ty + this.tx * val.m12 + this.ty * val.m22);
            }
            else if (Number(val)) {
                var num = Number(val);
                return $axure.utils.Matrix2D(this.m11 * num, this.m12 * num, this.m21 * num, this.m22 * num, this.tx * num, this.ty * num);
            }
            else
                return undefined;
        },
...
//# sourceMappingURL=axutils.js.map