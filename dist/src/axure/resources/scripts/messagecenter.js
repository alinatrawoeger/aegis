...
(function () {
    ...
    if ($axure.browser.isEdge)
        BROWSER_VERSION = Number(edgeMatch[1]);
    if (!$axure.browser.isEdge) {
        ...
        if (CHROME)
            BROWSER_VERSION = Number(chromeMatch[1]);
    }
    ...
    if (SAFARI)
        BROWSER_VERSION = Number(safariMatch[1]);
    ...
    if (FIREFOX)
        BROWSER_VERSION = Number(firefoxMatch[1]);
...
(function () {
    ...
    if (CHROME_5_LOCAL) {
        document.addEventListener("DOMContentLoaded", function () {
            ...
            $('#axureEventReceiverDiv')[0].addEventListener('axureMessageReceiverEvent', function () {
