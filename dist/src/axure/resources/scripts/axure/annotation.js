...
$axure.internal(function ($ax) {
    ...
    var _toggleAnnotationDialog = function (elementId, event) {
        ...
        var messageData = { id: elementId, x: x, y: y };
        if (!$axure.utils.isInPlayer())
            messageData.page = $ax.pageData.notesData;
    ...
    var _createFootnotes = $ax.annotation.createFootnotes = function (query, create) {
        if (!$ax.document.configuration.showAnnotations)
            return;
                ...
                query.each(function (dObj, elementId) {
                    ...
                    if (fns !== undefined) {
                        ...
                        if (create) {
                            ...
                            if (isMaster)
                                isVisible = dObj.visible;
                            else
                                isVisible = $ax.visibility.IsIdVisible(elementId);
                            if (!isVisible) {
                                var ann = document.getElementById(elementId + "_ann");
                                if (ann)
                                    $ax.visibility.SetVisible(ann, false);
                            }
                        }
                    }
                });
    ...
    var _adjustIconLocation = $ax.annotation.adjustIconLocation = function (id, dObj) {
        ...
        if (ref) {
            if (!corners)
                corners = $ax.public.fn.getCornersFromComponent(id);
