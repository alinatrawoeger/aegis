...
$axure.internal(function ($ax) {
    ...
    var isEqual = function (left, right, isFunction) {
        ...
        if (left instanceof Date && right instanceof Date) {
            if (left.getMilliseconds() != right.getMilliseconds())
                return false;
        ...
        if (left instanceof Object && right instanceof Object) {
            ...
            for (prop in left) {
                ...
                if (!isEqual(left[prop], right[prop], false))
                    return false;
            }
            for (prop in right) {
    ...
    _exprFunctions.ToLowerCase = function (value) {
        var str = _exprFunctions.ToString(value);
    ...
    _exprFunctions.SetWidgetFormText = function (elementIds, value) {
        for (var i = 0; i < elementIds.length; i++) {
            ...
            if (obj.val() == value || (value == '' && $ax.placeholderManager.isActive(elementId)))
                return;
            obj.val(value);
    ...
    _exprFunctions.SetFocusedWidgetText = function (elementId, value) {
        if (window.lastFocusedControl) {
            ...
            if ($ax.public.fn.IsTextBox(type) || $ax.public.fn.IsTextArea(type))
                ...
                _exprFunctions.SetWidgetRichText([elementId], value, true);
        }
    };
    _exprFunctions.GetRtfElementHeight = function (rtfElement) {
        if (rtfElement.innerHTML == '')
            rtfElement.innerHTML = '&nbsp;';
    ...
    var autofitWidget = function (id) {
        ...
        if (obj.autoFitHeight || obj.autoFitWidth) {
            ...
            if (obj.autoFitHeight) {
                ...
                if (style.paddingTop)
                    newHeight += Number(style.paddingTop);
                if (style.paddingBottom)
                    newHeight += Number(style.paddingBottom);
            ...
            if (obj.autoFitWidth) {
                ...
                if (style.paddingLeft)
                    newWidth += Number(style.paddingLeft);
                if (style.paddingRight)
                    newWidth += Number(style.paddingRight);
                if (newWidth - size.width != 0) {
                    css.width = newWidth;
                }
            }
            ;
            var deltaWidth = css.width - size.width;
            var deltaHeight = css.height - size.height;
            if (deltaWidth !== 0 || deltaHeight !== 0) {
                query.resize(css, {});
                var hAlign = style.horizontalAlignment;
                var vAlign = style.verticalAlignment;
                var dx = 0;
                var dy = 0;
                if (hAlign === "right") {
                    dx = -deltaWidth;
    ...
    _exprFunctions.GetWidgetText = function (ids) {
        if ($ax.placeholderManager.isActive(ids[0]))
            return '';
    ...
    _exprFunctions.GetWidgetValueLength = function (ids) {
        ...
        if (!id)
            return undefined;
        if ($ax.placeholderManager.isActive(id))
            return 0;
    ...
    _exprFunctions.GetWidgetRectangles = function (elementIds, eventInfo) {
        ...
        rects.lastRect = new $ax.drag.Rectangle(boundingRect.left, boundingRect.top, boundingRect.width, boundingRect.height);
    ...
    _exprFunctions.GetAdaptiveView = function (eventInfo) {
        if (eventInfo && eventInfo.srcElement) {
            ...
            if (diagramObject.owner.type == 'Axure:Master') {
                ...
                if (viewIdChain.length > 0)
                    return viewIdChain[viewIdChain.length - 1];
                else
                    return '19e82109f102476f933582835c373474';
            }
        }
