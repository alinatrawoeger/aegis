...
$axure.internal(function ($ax) {
    ...
    function IsNodeVisible(nodeId) {
        ...
        while (!$(current).hasClass("treeroot")) {
            if (!$ax.visibility.IsVisible(parent))
                return false;
            current = parent;
    ...
    $ax.tree.ExpandNode = function (nodeId, childContainerId, plusMinusId) {
        ...
        if (!container || $ax.visibility.IsVisible(container))
            return;
        ...
        while (!$(current).hasClass("treeroot")) {
            ...
            current = parent;
            parent = parent.parentNode;
            if (!isVisible && $ax.visibility.IsVisible(parent))
                break;
        }
    };
    $ax.tree.CollapseNode = function (nodeId, childContainerId, plusMinusId) {
        ...
        if (!container || !$ax.visibility.IsVisible(container))
            return;
        if (plusMinusId != '')
            $ax.style.SetWidgetSelected(plusMinusId, false);
        var delta = _getExpandCollapseDelta(nodeId, childContainerId);
    ...
    $ax.tree.InitializeTreeNode = function (nodeId, plusminusid, childContainerId, selectText) {
        ...
        if (childContainer) {
            ...
            if (isCollapsed)
                $ax.visibility.SetVisible(childContainer, false);
        ...
        if (plusminusid != '') {
            $jobj(plusminusid).click(function () {
                ...
                if (visibleSet)
                    ...
                    $ax.tree.ExpandNode(nodeId, childContainerId, plusminusid);
