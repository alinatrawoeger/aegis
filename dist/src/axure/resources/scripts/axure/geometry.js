    ...
    var linesIntersect = function (l1, l2) {
        ...
        if (l1Vert || l2Vert) {
            if (l1Vert && l2Vert) {
                ...
                if (l1p2.y < l2p1.y || l1p1.y > l2p2.y)
                    return false;
        ...
        if (m1 == m2) {
            // If the lines don't follow the same path, return
            if (l1p1.y != (l2p1.y + (l1p1.x - l2p1.x) * m1))
                return false;
            // if they never meet, return
            if (l1p2.x < l2p1.x || l1p1.x > l2p2.x)
                return false;
    ...
    var _checkInside = function (polygon, point) {
        ...
        for (i = 0; i < polygon.length; i++) {
            ...
            if (intersectInfo.parallel) {
                ...
                if (intersectInfo.parallel.first.x <= point.x && intersectInfo.parallel.second.x >= point.x)
                    return true;
                continue;
            }
            var intersectionX = intersectInfo.point.x;
            if (intersectionX > point.x || intersectionX < closest)
                continue;
            if (intersectionX == point.x)
                return true;
            // If closer than last time, reset cosine.
            if (intersectionX != closest)
                cos = -2;
