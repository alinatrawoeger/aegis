...
$axure.internal(function ($ax) {
            ...
            (function (axEventObject) {
                return function (e) {
                    if (e.type == 'mouseenter' && doMouseEnterGuard) {
                        if (mouseEnterGuard[elementId])
                            return;
                        else
                            mouseEnterGuard[elementId] = true;
                    }
    ...
    var _createProxies = function (diagramObject, elementId) {
        ...
        if (!createFocus && !createLostFocus)
            return;
        if (!diagramObject.interactionMap)
            diagramObject.interactionMap = {};
        if (createFocus)
            diagramObject.interactionMap.onFocus = { proxy: true, eventType: 'OnFocus' };
        if (createLostFocus)
            diagramObject.interactionMap.onLostFocus = { proxy: true, eventType: 'OnLostFocus' };
    };
    ...
    var _notAllowedInvisible = function (id) {
        ...
        if ($ax.public.fn.IsReferenceDiagramObject(type) || $ax.public.fn.IsLayer(type))
            return false;
    ...
    _event.addSuppressedEvent = function (id, event) {
        if (!_suppressedEvents[id])
            _suppressedEvents[id] = [];
    ...
    var _handleEvent = $ax.event.handleEvent = function (elementId, eventInfo, axEventObject, skipShowDescriptions, synthetic) {
        ...
        if (_enteredWidgets[elementId] && eventType == 'OnMouseEnter')
            return; // Suppress entering a widget when already in widget (ie only)
        ...
        eventNesting += 1;
        if (!synthetic && !_getCanClick() && (eventType == 'OnClick' || eventType == 'OnPageClick'))
            return;
        ...
        if (!synthetic && currentEvent && currentEvent.originalEvent && currentEvent.originalEvent.handled && !eventInfo.isMasterEvent)
            return;
        if (!synthetic && elementId && !$ax.style.getObjVisible(elementId) && _notAllowedInvisible(elementId))
            return;
        //if debug
        ...
        if (!skipShowDescriptions || eventType == 'OnPageLoad')
            postMessage('axEvent', { 'label': axObjLabel, 'type': axObjType, 'event': axEventObject });
        var bubble = true;
            ...
            for (var i = 0; i < axEventObject.cases.length; i++) {
                ...
                for (var j = 0; j < currentCase.actions.length; j++) {
                    ...
                    if (action.action == 'wait')
                        break;
                    if (action.action != 'refreshRepeater')
                        continue;
    ...
    var _handleScrollEvent = function (elementId, eventInfo, originalEvent, scrolledUp, scrolledDown, interactionMap, skipShowDescription, synthetic) {
        ...
        if (interactionMap.onScroll)
            _handleEvent(elementId, eventInfo, interactionMap.onScroll, skipShowDescription, synthetic);
    ...
    var _showCaseDescriptions = function (elementId, eventInfo, axEventObject, synthetic) {
        if (axEventObject.cases.length == 0)
            return true;
    ...
    var _shouldShowCaseDescriptions = function (axEventObject) {
        ...
        if (_isEventSimulating(axEventObject))
            return false;
        if (axEventObject.cases.length >= 2)
            return true;
        return false;
    };
    var _isEventSimulating = function (axEventObject) {
        for (var i = 0; i < axEventObject.cases.length; i++) {
            if (axEventObject.cases[i].condition)
                return true;
        }
        return false;
    };
                ...
                for (var j = 0; j < currentCase.actions.length; j++) {
                    ...
                    for (var k = 0; k < action.repeatersToRefresh.length; k++) {
                        ...
                        if (id) {
                            ...
                            if (index != -1)
                                $ax.splice(groupRefreshes, index);
                        }
                    }
                }
    ...
    var _attachDefaultObjectEvent = function (elementIdQuery, elementId, eventName, fn) {
        var func = function (e) {
            if ($ax.style.IsWidgetDisabled(elementId) || _shouldIgnoreLabelClickFromCheckboxOrRadioButton(e))
                return true;
            return fn.apply(this, arguments);
        };
        var bind = !elementIdQuery[eventName];
        if (bind)
            elementIdQuery.bind(eventName, func);
        else
            elementIdQuery[eventName](func);
    };
    ...
    var _attachCustomObjectEvent = function (elementId, eventName, fn) {
        ...
        if (!handlers)
            _objectIdToEventHandlers[elementId] = handlers = {};
        var fnList = handlers[eventName];
        if (!fnList)
            handlers[eventName] = fnList = [];
    ...
    _event.layerMapFocus = function (layer, elementId) {
        ...
        if ($ax.public.fn.IsLayer(mainObj.type)) {
            ...
            if (baseId)
                _layerToFocusableWidget[elementId] = baseId;
            return;
        }
    ...
    var _needsProxy = function (obj, id, proxyName) {
        ...
        if (obj.interactionMap && obj.interactionMap[proxyName])
            return false;
        var parentId = _widgetToFocusParent[id];
        if (parentId)
            return _needsProxyHelper(parentId, proxyName);
        return false;
    };
    ...
    var _initializeObjectEvents = function (query, refreshType) {
        ...
        query.each(function (dObj, elementId) {
            if (dObj == null)
                return; // TODO: Update expo items that pass here to potentially remove this logic
            var $element = $jobj(elementId);
            var itemId = $ax.repeater.getItemIdFromElementId(elementId);
            var isItem = itemId && $ax.public.fn.IsRepeater(dObj.type);
            ...
            if (!itemId || itemReset) {
                ...
                var isError = $element.hasClass('error');
            ...
            //    }
            //};
            var isInput = $ax.public.fn.IsTextArea(dObj.type) || $ax.public.fn.IsTextBox(dObj.type);
            if (isInput) {
            ...
            var dynamicPanelMouseOver = function (elementId, fromChild) {
                ...
                if ($.inArray(elementId, _event.mouseOverIds) != -1)
                    return;
                    ...
                    (function (currCount) {
                        window.setTimeout(function () {
                            if (currCount == hoverMouseCount)
                                _raiseSyntheticEvent(elementId, 'onMouseHover', false, hoverEventInfo, true);
                        }, MIN_HOVER_HOLD_TIME);
                    })(hoverMouseCount);
                    ...
                    (function (currCount) {
                        ...
                        window.setTimeout(function () {
                            if (currCount == longClickMouseCount)
                                _raiseSyntheticEvent(elementId, 'onLongClick', false, eventInfo, true);
            ...
            if ($ax.event.HasKeyUpOrDown(dObj))
            ...
            _event.initMobileEvents(hasTap ? $element : $(), hasMove ? $element : $(), elementId);
            //attach link alternate styles
            if (dObj.type == 'hyperlink') {
                $element.mouseenter(function () {
                    var linkId = this.id;
                    if (_event.mouseOverIds.indexOf(linkId) != -1)
                        return true;
                    _event.mouseOverIds[_event.mouseOverIds.length] = linkId;
                    var mouseOverObjectId = _event.mouseOverObjectId;
                    if (mouseOverObjectId && $ax.style.IsWidgetDisabled(mouseOverObjectId))
                        return true;
                    ...
                    var linkId = this.id;
                    var mouseOverObjectId = _event.mouseOverObjectId;
                    if ($ax.style.IsWidgetDisabled(mouseOverObjectId))
                        return undefined;
            ...
            if (dObj.type == 'inlineFrame') {
                ...
                if (url)
                    $jobj($ax.INPUT(elementId)).attr('src', url);
            }
            ;
        });
    };
    ...
    function clearMouseDownIxStyle(e) {
        if (_event.mouseDownObjectId) {
            $('#' + _event.mouseDownObjectId).trigger({ type: "mouseup",
    ...
    var _attachIxStyleEvents = function (dObj, elementId, $element, ignoreHasIxStyles) {
        ...
        if (needsMouseFilter) {
            ...
            $element.on($ax.features.eventNames.mouseDownName + '.ixStyle', function () {
                ...
                if (!$ax.style.ObjHasMouseDown(elementId))
                    return;
    ...
    (function () {
        ...
        $ax.event.initKeyEvents = function ($query) {
            $query.keydown(function (e) {
                ...
                $ax.setjBrowserEvent(e);
                if (!elementId)
                    fireEventThroughContainers('onKeyDown', undefined, false, [$ax.constants.PAGE_TYPE, $ax.constants.REFERENCE_DIAGRAM_OBJECT_TYPE, $ax.constants.DYNAMIC_PANEL_TYPE, $ax.constants.REPEATER], [$ax.constants.PAGE_TYPE, $ax.constants.REFERENCE_DIAGRAM_OBJECT_TYPE, $ax.constants.LAYER_TYPE]);
                else
            ...
            $query.keyup(function (e) {
                ...
                if (!elementId)
                    fireEventThroughContainers('onKeyUp', undefined, false, [$ax.constants.PAGE_TYPE, $ax.constants.REFERENCE_DIAGRAM_OBJECT_TYPE, $ax.constants.DYNAMIC_PANEL_TYPE, $ax.constants.REPEATER], [$ax.constants.PAGE_TYPE, $ax.constants.REFERENCE_DIAGRAM_OBJECT_TYPE, $ax.constants.LAYER_TYPE]);
                else
        ...
        _event.initMobileEvents = function ($tapQuery, $moveQuery, elementId) {
            ...
            $tapQuery.bind('touchstart', function (e) {
                ...
                if (time - lastClickEventTime < DBLCLICK_THRESHOLD_MS) {
                    ...
                    if (axEventObject)
                        e.preventDefault(); //for Chrome on Android
                }
    ...
    var _updateMouseLocation = function (e, end) {
        if (!e)
            return;
        if (IE_10_AND_BELOW && typeof (e.type) == 'unknown')
            return;
        if (e.type != 'mousemove' && e.type != 'touchstart' && e.type != 'touchmove' && e.type != 'touchend'
            && e.type != 'pointermove' && e.type != 'pointerdown' && e.type != 'pointerup')
            return;
        var newX;
        var newY;
        if (IE_10_AND_BELOW) {
        ...
        if (_mouseLocation.x == newX && _mouseLocation.y == newY)
            return;
    ...
    var _raiseSelectedEvents = function (elementId, value) {
        ...
        if (value)
            $ax.event.raiseSyntheticEvent(elementId, 'onSelect');
        else
            $ax.event.raiseSyntheticEvent(elementId, 'onUnselect');
    };
    $ax.event.raiseSelectedEvents = _raiseSelectedEvents;
    var _raiseErrorEvents = function (elementId, value) {
        if (value)
            $ax.event.raiseSyntheticEvent(elementId, 'onErrorSet');
        else
            $ax.event.raiseSyntheticEvent(elementId, 'onErrorRemoved');
    };
    ...
    $ax.event.HasIxStyles = function (diagramObject) {
        ...
        if (diagramObject.adaptiveStyles) {
            for (var viewId in diagramObject.adaptiveStyles) {
                if (diagramObject.adaptiveStyles[viewId].stateStyles)
                    return true;
            }
        }
        return false;
    ...
    $ax.event.HasTextChanged = function (diagramObject) {
        if (!$ax.public.fn.IsTextBox(diagramObject.type) && !$ax.public.fn.IsTextArea(diagramObject.type))
            return false;
    ...
    $ax.event.TryFireTextChanged = function (elementId) {
        ...
        if (!$ax.hasElementTextChanged(elementId, query.val()))
            return;
    ...
    $ax.event.HasSelectionChanged = function (diagramObject) {
        if (!$ax.public.fn.IsListBox(diagramObject.type) && !$ax.public.fn.IsComboBox(diagramObject.type))
            return false;
        var map = diagramObject.interactionMap;
    ...
    $ax.event.HasCheckedChanged = function (diagramObject) {
        if (!$ax.public.fn.IsCheckBox(diagramObject.type) && !$ax.public.fn.IsRadioButton(diagramObject.type))
            return false;
        var map = diagramObject.interactionMap;
    ...
    var fireEventForPageOrMaster = function (elementId, eventName, interactionMap, isPage, skipShowDescription, synthetic) {
        ...
        if (axEvent) {
            ...
            if (scrolling)
                _handleScrollEvent(elementId, eventInfo, currentEvent.originalEvent, _event.windowScrollingUp, _event.windowScrollingDown, interactionMap, skipShowDescription, synthetic);
            else
                _handleEvent(elementId, eventInfo, axEvent, skipShowDescription, synthetic);
        }
    };
    ...
    var fireEventThroughContainers = function (eventName, objects, synthetic, searchFilter, callFilter, path, itemId) {
        ...
        if (!objects) {
            ...
            if (searchFilter.indexOf($ax.constants.PAGE_TYPE) != -1)
                fireEventThroughContainers(eventName, $ax.pageData.page.diagram.objects, synthetic, searchFilter, callFilter);
        ...
        for (var i = 0; i < objects.length; i++) {
            ...
            if ($ax.public.fn.IsReferenceDiagramObject(obj.type)) {
                ...
                if (searchFilter.indexOf($ax.constants.REFERENCE_DIAGRAM_OBJECT_TYPE) != -1)
                    fireEventThroughContainers(eventName, $ax.pageData.masters[obj.masterId].diagram.objects, synthetic, searchFilter, callFilter, pathCopy, itemId);
            }
            else if ($ax.public.fn.IsDynamicPanel(obj.type)) {
                if (_callFilterCheck(callFilter, $ax.constants.DYNAMIC_PANEL_TYPE))
                    $ax.event.raiseSyntheticEvent(objId, eventName, skipShowDescription, undefined, !synthetic);
