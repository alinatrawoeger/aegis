...
(function () {
    ...
    $axure.internal = function (initFunction) {
        ...
        if (!$ax.messageCenter)
            $ax.messageCenter = $axure.messageCenter;
    ...
    var _fireResize = function () {
        ...
        for (var i = 0; i < _resizeFunctions.length; i++)
            _resizeFunctions[i]();
    };
    $axure.resize = function (fn) {
        if (fn)
            _resizeFunctions[_resizeFunctions.length] = fn;
        else
            $(window).resize();
    };
    ...
    var _getFilterFnFromQuery = function (query) {
        var filter = function (diagramObject, elementId) {
            ...
            if (diagramObject && !$ax.public.fn.IsReferenceDiagramObject(diagramObject.type) && !document.getElementById(elementId))
                return false;
            var retVal = true;
    ...
    $ax.public.fn.filter = function (query, queryArg) {
        ...
        if (this.query.elementId)
            returnVal.query.elementId = this.query.elementId;
    ...
    $ax.public.fn.getElements = function () {
        ...
        this.each(function (dObj, elementId) {
            ...
            if (elementById)
                elements[elements.length] = elementById;
        });
        return elements;
    };
    ...
    $ax.public.fn.getParents = function (deep, filter) {
        ...
        var getParent = function (elementId) {
            ...
            if (parentRepeater == scriptId) {
                ...
                if (itemNum)
                    return filter.indexOf('repeater') != -1 ? scriptId : getParent(scriptId);
    ...
    $ax.public.fn.getChildren = function (deep, ignoreUnplaced) {
        ...
        var getChildren = function (elementId) {
            ...
            if (isRepeater) {
                ...
                parent = $jobj($ax.repeater.applySuffixToElementId(elementId, '_children'));
    ...
    var _fixForBasicLinks = function (query) {
        ...
        for (var i = 0; i < query.length; i++) {
            var child = $(query[i]);
            if (child.hasClass('basiclink'))
                retval = retval.add(child.children());
            else
                retval = retval.add(child);
        }
        return retval;
    };
})();
//# sourceMappingURL=axQuery.js.map