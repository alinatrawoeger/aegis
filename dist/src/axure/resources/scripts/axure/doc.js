$axure.internal(function ($ax) {
    ...
    (function () {
        ...
        _initializePageData = function () {
            if (!_pageData || !_pageData.page || !_pageData.page.diagram)
                return;
            var objIdToObject = {};
            ...
            var mapScriptIdToRepeaterId = function (scriptId, repeaterId) {
                ...
                if (scriptIds)
                    scriptIds[scriptIds.length] = scriptId;
                else
                    repeaterIdToScriptIds[repeaterId] = [scriptId];
            };
            ...
            var mapIdsToRepeaterAndLayer = function (path, objs, repeaterId) {
                ...
                for (var i = 0; i < objs.length; i++) {
                    ...
                    if (repeaterId)
                        mapScriptIdToRepeaterId(scriptId, repeaterId);
        ...
        var _getEventInfoFromEvent = function (event, skipShowDescriptions, elementId) {
            ...
            if (event != null) {
                ...
                if (mouseEvent && !mouseEvent.type)
                    mouseEvent.type = event.type;
                if (skipShowDescriptions)
                    eventInfo.skipShowDescriptions = true;
        ...
        $ax.GetShapeIdFromText = function (id) {
            if (!id)
                return undefined; // this is to prevent an infinite loop.
        ...
        $ax.GetImageIdFromShape = function (id) {
            ...
            if (!image.length)
                image = $jobj(id).find('img[id$=image_sketch]');
        ...
        $ax.addItemIdToRepeater = function (itemId, repeaterId) {
            ...
            if (itemIds)
                ...
                repeaterIdToItemIds[repeaterId] = [itemId];
            var scriptIds = repeaterIdToScriptIds[repeaterId];
            for (var i = 0; i < scriptIds.length; i++)
                elementIdToObject[$ax.repeater.createElementId(scriptIds[i], itemId)] = $ax.getObjectFromScriptId(scriptIds[i]);
        };
        $ax.getAllElementIds = function () {
            ...
            for (var i = 0; i < _scriptIds.length; i++) {
                ...
                if (repeaterId && repeaterId != scriptId) {
                    ...
                    elementIds[elementIds.length] = scriptId;
            }
            return elementIds;
        };
