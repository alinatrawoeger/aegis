$axure.internal(function ($ax) {
    ...
    $ax.public.fn.removeCompound = function (jobj) { if (_isCompoundVectorHtml(jobj[0]))
        jobj.removeClass('compound'); };
    $ax.public.fn.restoreCompound = function (jobj) { if (_isCompoundVectorHtml(jobj[0]))
        jobj.addClass('compound'); };
    $ax.public.fn.compoundIdFromComponent = function (id) {
        ...
        if (pPos < 1)
            ...
            return id.substring(0, pPos);
        else
            return id.substring(0, pPos) + id.substring(dashPos);
    };
    ...
    $ax.public.fn.convertToSingleImage = function (jobj) {
        ...
        //var layer = 
        if (!_isCompoundVectorHtml(jobj[0]))
            return;
    ...
    $ax.public.fn.inversePathLengthFunction = function (pathFunction) {
        ...
        var labelCumulativeLength = function (node) {
            if (!node.LowerChild) {
                ...
                labelCumulativeLength(node.LowerChild);
            if (!node.HigherChild) {
                node.Cumulative = cumulative;
                ...
                labelCumulativeLength(node.HigherChild);
        };
        var getIntervalFromPathLength = function (node, length) {
            if (length < node.Cumulative) {
                ...
                return node.HigherChild ? getIntervalFromPathLength(node.HigherChild, length) : node.HigherInterval;
        };
