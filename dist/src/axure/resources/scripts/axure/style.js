$axure.internal(function ($ax) {
    ...
    var _widgetHasState = function (id, state) {
        ...
        for (var i = 0; i < adaptiveIdChain.length; i++) {
            ...
            if (adaptiveStyle && adaptiveStyle.stateStyles && adaptiveStyle.stateStyles[state])
                return true;
        }
    ...
    $ax.style.SetWidgetHover = function (id, value) {
        if (!_widgetHasState(id, MOUSE_OVER))
            return;
        var valToSet = value || _isRolloverOverride(id);
        var state = _generateFullState(id, undefined, undefined, undefined, undefined, valToSet);
    ...
    $ax.style.SetWidgetMouseDown = function (id, value, checkMouseOver) {
        if ($ax.style.IsWidgetDisabled(id))
            return;
        if (!_widgetHasState(id, MOUSE_DOWN))
            return;
        var state = _generateFullState(id, undefined, undefined, undefined, undefined, !checkMouseOver ? true : undefined, value);
        _applyImageAndTextJson(id, state);
        _updateElementIdImageStyle(id, state);
    };
    var _generateFullState = function (id, forceFocused, forceSelected, forceDisabled, forceError, forceMouseOver, forceMouseDown) {
        var state = $ax.style.ShowingHint(id) ? HINT : '';
    ...
    var _compoundStates = function (current, baseState) {
        ...
        if (baseState.length < 1)
            return current;
        var capital = current.charAt(0).toUpperCase() + current.slice(1);
        return baseState + capital;
    };
    $ax.style.updateImage = function (id) {
        var state = $ax.style.generateState(id);
        var imageUrl = $ax.adaptive.getImageForStateAndView(id, state);
        if (imageUrl)
            _applyImage(id, imageUrl, state);
        $ax.style.updateElementIdImageStyle(id, state);
    };
    $ax.style.SetWidgetFocused = function (id, value) {
        if (_isWidgetDisabled(id))
            return;
        if (!_widgetHasState(id, FOCUSED))
            return;
        var state = _generateFullState(id, value);
        _applyImageAndTextJson(id, state);
        _updateElementIdImageStyle(id, state);
    };
    $ax.style.SetWidgetSelected = function (id, value, alwaysApply) {
        ...
        if (value) {
            ...
            if (group) {
                $("[selectiongroup='" + group + "']").each(function () {
                    ...
                    if (otherId == id)
                        return;
                    if ($ax.visibility.isScriptIdLimbo($ax.repeater.getScriptIdFromElementId(otherId)))
                        return;
            ...
            if ($ax.public.fn.IsDynamicPanel(obj.type) || $ax.public.fn.IsLayer(obj.type)) {
                ...
                for (var i = 0; i < children.length; i++) {
                    ...
                    if (value) {
                        ...
                        if (group)
                            for (var _i = 0, _a = $("[selectiongroup='" + group + "']"); _i < _a.length; _i++) {
                                var item = _a[_i];
                                skipIds.add(item.id);
                            }
                    }
                    ...
                    if (childObj.hasClass('treeroot')) {
                        ...
                        $axure('#' + childId).selected(value);
                }
            }
            else {
                var widgetHasSelectedState = _widgetHasState(id, SELECTED);
                while (obj.isContained && !widgetHasSelectedState)
                    obj = obj.parent;
                ...
                if (alwaysApply || widgetHasSelectedState) {
                    var state = _generateFullState(actionId, undefined, value);
                ...
                while (obj.isContained && !$ax.getObjectFromElementId(id).interactionMap)
                    obj = obj.parent;
        ...
        if (raiseSelectedEvents)
            $ax.event.raiseSelectedEvents(actionId, value);
    };
    ...
    $ax.style.SetWidgetEnabled = function (id, value) {
        ...
        if (!_widgetHasState(id, DISABLED))
            return;
        if (!value) {
            var state = _generateFullState(id, undefined, undefined, true);
            _applyImageAndTextJson(id, state);
            _updateElementIdImageStyle(id, state);
        }
        else
    ...
    $ax.style.SetWidgetError = function (id, value) {
        ...
        if (raiseErrorEvents)
            $ax.event.raiseErrorEvents(id, value);
        if (!_widgetHasState(id, ERROR) && !_hasAnyErrorClass(id))
            return;
        var state = _generateFullState(id, undefined, undefined, undefined, value);
        _applyImageAndTextJson(id, state);
        _updateElementIdImageStyle(id, state);
    };
    ...
    $ax.style.SetWidgetPlaceholder = function (id, active, text, password) {
        ...
        obj.val(text);
        var style = _computeAllOverrides(id, undefined, state, $ax.adaptive.currentViewId);
        if (!$.isEmptyObject(style))
            _applyTextStyle(inputId, style);
        _updateStateClasses([
            id,
    ...
    $ax.style.mapElementIdToImageOverrides = function (elementId, override) {
        for (var key in override)
            _addImageOverride(elementId, key, override[key]);
    };
    ...
    var _highestPriorityBaseState = _style.highestPriorityBaseState = function (state) {
        if (state == SELECTED_ERROR)
            return ERROR;
        if (state == SELECTED_HINT)
            return HINT;
        if (state == SELECTED_ERROR_HINT)
            return HINT;
        if (state == ERROR_HINT)
            return HINT;
        if (state == MOUSE_OVER_MOUSE_DOWN)
            return MOUSE_DOWN;
        if (state == MOUSE_OVER_SELECTED)
            return SELECTED;
        if (state == MOUSE_OVER_ERROR)
            return ERROR;
        if (state == MOUSE_OVER_HINT)
            return HINT;
        if (state == MOUSE_OVER_SELECTED_ERROR)
            return ERROR;
        if (state == MOUSE_OVER_SELECTED_HINT)
            return HINT;
        if (state == MOUSE_OVER_SELECTED_ERROR_HINT)
            return HINT;
        if (state == MOUSE_OVER_ERROR_HINT)
            return HINT;
        if (state == MOUSE_DOWN_SELECTED)
            return SELECTED;
        if (state == MOUSE_DOWN_ERROR)
            return ERROR;
        if (state == MOUSE_DOWN_HINT)
            return HINT;
        if (state == MOUSE_DOWN_SELECTED_ERROR)
            return ERROR;
        if (state == MOUSE_DOWN_SELECTED_HINT)
            return HINT;
        if (state == MOUSE_DOWN_SELECTED_ERROR_HINT)
            return HINT;
        if (state == MOUSE_DOWN_ERROR_HINT)
            return HINT;
        if (state == MOUSE_OVER_MOUSE_DOWN_SELECTED)
            return SELECTED;
        if (state == MOUSE_OVER_MOUSE_DOWN_ERROR)
            return ERROR;
        if (state == MOUSE_OVER_MOUSE_DOWN_HINT)
            return HINT;
        ...
        if (state == FOCUSED_ERROR)
            return ERROR;
        if (state == FOCUSED_HINT)
            return HINT;
        if (state == FOCUSED_SELECTED_ERROR)
            return ERROR;
        if (state == FOCUSED_SELECTED_HINT)
            return HINT;
        if (state == FOCUSED_SELECTED_ERROR_HINT)
            return HINT;
        if (state == FOCUSED_ERROR_HINT)
            return HINT;
        if (state == HINT_DISABLED)
            return DISABLED;
        if (state == SELECTED_DISABLED)
            return DISABLED;
        if (state == ERROR_DISABLED)
            return DISABLED;
        if (state == SELECTED_HINT_DISABLED)
            return DISABLED;
        if (state == ERROR_HINT_DISABLED)
            return DISABLED;
        if (state == SELECTED_ERROR_DISABLED)
            return DISABLED;
        if (state == SELECTED_ERROR_HINT_DISABLED)
            return DISABLED;
        return state;
    };
    // we need this for when we are looking for the correct image to apply
    var _decomposeState = _style.decomposeState = function (state) {
        ...
        if (state == MOUSE_OVER_HINT)
            return MOUSE_OVER;
        if (state == MOUSE_OVER_SELECTED_ERROR)
            return MOUSE_OVER_SELECTED;
        if (state == MOUSE_OVER_SELECTED_HINT)
            return MOUSE_OVER_SELECTED;
        if (state == MOUSE_OVER_SELECTED_ERROR_HINT)
            return MOUSE_OVER_SELECTED_ERROR;
        if (state == MOUSE_OVER_ERROR_HINT)
            return MOUSE_OVER_ERROR;
        if (state == MOUSE_DOWN_SELECTED)
            return MOUSE_DOWN;
        if (state == MOUSE_DOWN_ERROR)
            return MOUSE_DOWN;
        if (state == MOUSE_DOWN_HINT)
            return MOUSE_DOWN;
        if (state == MOUSE_DOWN_SELECTED_ERROR)
            return MOUSE_DOWN_SELECTED;
        if (state == MOUSE_DOWN_SELECTED_HINT)
            return MOUSE_DOWN_SELECTED;
        if (state == MOUSE_DOWN_SELECTED_ERROR_HINT)
            return MOUSE_DOWN_SELECTED_ERROR;
        if (state == MOUSE_DOWN_ERROR_HINT)
            return MOUSE_DOWN_ERROR;
        if (state == MOUSE_OVER_MOUSE_DOWN_SELECTED)
            return MOUSE_OVER_MOUSE_DOWN;
        if (state == MOUSE_OVER_MOUSE_DOWN_ERROR)
            return MOUSE_OVER_MOUSE_DOWN;
        if (state == MOUSE_OVER_MOUSE_DOWN_HINT)
            return MOUSE_OVER_MOUSE_DOWN;
        ...
        if (state == SELECTED_ERROR_DISABLED)
            return SELECTED_ERROR;
        if (state == SELECTED_ERROR_HINT_DISABLED)
            return SELECTED_ERROR_HINT;
        return NORMAL;
    };
    var _updateElementIdImageStyle = _style.updateElementIdImageStyle = function (elementId, state) {
        if (!_style.elementHasAnyImageOverride(elementId))
            return;
        if (!state)
            state = _generateState(elementId);
    ...
    var _updateStateClasses = function (ids, event, addMouseOverOnMouseDown) {
        for (var i = 0; i < ids.length; i++) {
            _updateStateClassesHelper(ids[i], event, addMouseOverOnMouseDown);
        }
    };
    var _updateStateClassesHelper = function (id, event, addMouseOverOnMouseDown) {
        var jobj = $jobj(id);
        var isMouseDownEvent = _stateHasMouseDown(event);
        for (var i = 0; i < ALL_STATES_WITH_CSS_CLASS.length; i++)
            jobj.removeClass(ALL_STATES_WITH_CSS_CLASS[i]);
        if (addMouseOverOnMouseDown && isMouseDownEvent || _stateHasMouseOver(event))
            jobj.addClass(MOUSE_OVER);
        if (isMouseDownEvent)
            jobj.addClass(MOUSE_DOWN);
        if (_stateHasFocused(event))
            jobj.addClass(FOCUSED);
        if (_stateHasSelected(event))
            jobj.addClass(SELECTED);
        if (_stateHasError(event))
            jobj.addClass(ERROR);
        if (_stateHasDisabled(event))
            jobj.addClass(DISABLED);
        if (_stateHasHint(event))
            jobj.addClass(HINT);
    };
    /* -------------------
    ...
    var _computeAllOverrides = $ax.style.computeAllOverrides = function (id, parentId, state, currentViewId) {
        var computedStyle = {};
        if (parentId)
            computedStyle = _computeAllOverrides(parentId, null, state, currentViewId);
        var diagramObject = $ax.getObjectFromElementId(id);
    ...
    var _computeStateStyleForViewChain = function (diagramObject, state, viewIdChain, excludeNormal) {
        ...
        for (var i = viewIdChain.length - 1; i >= 0; i--) {
            ...
            if (stateStyle)
                return $.extend({}, stateStyle);
            else if (viewStyle && viewStyle.stateStyles)
                return {}; //stateStyles are overriden but states could be null
        }
    ...
    $ax.style.reselectElements = function () {
        ...
        for (var id in _selectedWidgets) {
            ...
            if (!_selectedWidgets[id] || _hasAnySelectedClass(id))
                continue;
            var state = _generateFullState(id, undefined, true);
            _applyImageAndTextJson(id, state);
        }
        for (var id in _disabledWidgets) {
            // Only looking for the disabled widgets that don't have their class yet
            if (!_disabledWidgets[id] || _hasAnyDisabledClass(id))
                continue;
            var state = _generateFullState(id, undefined, undefined, true);
            _applyImageAndTextJson(id, state);
        }
    ...
    var _applyCssProps = function (element, styleProperties, applyAllStyle) {
        if (applyAllStyle) {
            ...
            for (var prop in allProps)
                element.style[prop] = allProps[prop];
        }
        else {
            var nodeName = element.nodeName.toLowerCase();
            if (nodeName == 'p') {
                var parProps = styleProperties.parProps;
                for (prop in parProps)
                    element.style[prop] = parProps[prop];
            }
            else if (nodeName != 'a') {
                var runProps = styleProperties.runProps;
                for (prop in runProps)
                    element.style[prop] = runProps[prop];
            }
        }
    };
    ...
    var _getCssStyleProperties = function (style) {
        ...
        toApply.allProps = {};
        if (style.fontName)
            toApply.allProps.fontFamily = toApply.runProps.fontFamily = style.fontName;
        ...
        if (style.fontSize)
            toApply.allProps.fontSize = toApply.runProps.fontSize = toApply.parProps.fontSize = style.fontSize;
        if (style.fontWeight !== undefined)
            toApply.allProps.fontWeight = toApply.runProps.fontWeight = style.fontWeight;
        ...
        if (style.strikethrough !== undefined) {
            ...
            if (style.strikethrough)
                textDecoration[index] = 'line-through';
            else if (index == 0)
        ...
        if (style.horizontalAlignment)
            toApply.allProps.textAlign = toApply.parProps.textAlign = toApply.runProps.textAlign = style.horizontalAlignment;
        if (style.lineSpacing)
            toApply.allProps.lineHeight = toApply.parProps.lineHeight = style.lineSpacing;
        if (style.textShadow)
            toApply.allProps.textShadow = toApply.parProps.textShadow = _getCssShadow(style.textShadow);
        if (style.letterCase)
            toApply.allProps.textTransform = toApply.parProps.textTransform = style.letterCase;
        if (style.characterSpacing)
            toApply.allProps.letterSpacing = toApply.runProps.letterSpacing = style.characterSpacing;
        return toApply;
    };
    ...
    $ax.style.CacheOriginalText = function (textId, hasRichTextBeenSet) {
        ...
        if (rtfQuery.length > 0) {
            ...
            rtfQuery.find('*').each(function (index, element) {
                ...
                if (!elementId)
                    element.id = elementId = 'cache' + CACHE_COUNTER++;
    ...
    $ax.style.prefetch = function () {
        ...
        for (var i = 0; i < scriptIds.length; i++) {
            ...
            if (!$ax.public.fn.IsImageBox(obj.type))
                continue;
