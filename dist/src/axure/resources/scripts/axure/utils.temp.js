...
$axure.internal(function ($ax) {
    ...
    var _deepCopy = function (original, trackCopies, filter) {
        if (trackCopies) {
            ...
            if (index != -1)
                return _originalToCopy[index][1];
        }
        ...
        var copy = isArray ? [] : {};
        if (trackCopies)
            _originalToCopy.push([original, copy]);
    ...
    var _getCopyIndex = function (original) {
        for (var i = 0; i < _originalToCopy.length; i++)
            if (original === _originalToCopy[i][0])
                return i;
        return -1;
    };
    ...
    var deepCopyObject = function (original, trackCopies, copy, filter) {
        for (var key in original) {
            if (!original.hasOwnProperty(key))
                continue; // Continue if the prop was not put there like a dictionary, but just a native part of the object
            if (filter && filter.indexOf[key] != -1)
                copy[key] = original[key]; // If that key is filtered out, skip recursion on it.
            else
                copy[key] = _deepCopy(original[key], trackCopies, filter);
        }
    };
...
$axure.internal(function ($ax) {
    $(window.document).ready(function () {
        if (!$ax.document.configuration.linkFlowsToPages && !$ax.document.configuration.linkFlowsToPagesNewWindow)
            return;
...
//# sourceMappingURL=utils.temp.js.map