$axure.internal(function ($ax) {
    ...
    $ax.drag.StartDragWidget = function (event, id) {
        ...
        if (event.donotdrag || widgetDragInfo.started)
            return;
        var x, y;
        var tg;
        if (IE_10_AND_BELOW) {
    ...
    $ax.drag.GetDragX = function () {
        if (widgetDragInfo.hasDragged)
            return widgetDragInfo.xDelta;
        return 0;
    };
    $ax.drag.GetDragY = function () {
        if (widgetDragInfo.hasDragged)
            return widgetDragInfo.yDelta;
        return 0;
    };
    ...
    $ax.drag.LogMovedWidgetForDrag = function (id, dragInfo) {
        ...
        if (dragInfo.hasDragged) {
            ...
            if (containerIndex != -1)
                id = id.substring(0, containerIndex);
    ...
    Rectangle.prototype.IntersectsWith = function (rect) {
        ...
        if (rect.length) {
            for (var i = 0; i < rect.length; i++)
                if (!rect[i].Invalid && this.IntersectsWith(rect[i]))
                    return true;
            return false;
        }
        if (rect.Invalid())
