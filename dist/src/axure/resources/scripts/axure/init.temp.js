$axure.internal(function ($ax) {
    $(window.document).ready(function () {
        ...
        function pushNextPrefix() {
            if (fnPrefix.length == 0)
                fnPrefix = 'A';
            else
                fnPrefix = fnPrefix[0] == 'Z' ? 'A'.repeat(fnPrefix.length + 1) : String.fromCharCode(fnPrefix.charCodeAt(0) + 1).repeat(fnPrefix.length);
        }
        function populateNotes(pageForNotes) {
            for (var master in pageForNotes.masters) {
                ...
                if ((master.notes && !$.isEmptyObject(masterData.notes)) || hasWidgetNotes) {
                    ...
                    if (hasWidgetNotes)
                        populateOwnerToFn(m.widgetNotes);
                }
        ...
        function populateOwnerToFn(widgetNotes) {
            if (typeof widgetNotes == 'undefined')
                return false;
...
//# sourceMappingURL=init.temp.js.map