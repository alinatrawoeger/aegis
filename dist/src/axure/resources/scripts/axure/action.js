$axure.internal(function ($ax) {
    ...
    var _addAnimations = function (animations) {
        ...
        for (var i = 0; i < animations.length; i++) {
            ...
            if (thisReady)
                ...
                if (!groups)
                    typeToGroups[animation.type] = groups = [];
    ...
    var _fireAnimationFromQueue = _action.fireAnimationFromQueue = function (id, type) {
        ...
        if (func && !_checkFireActionGroup(id, type, func))
            func();
    };
    ...
    var _getIdToResizeMoveState = function (eventInfo) {
        if (!eventInfo.idToResizeMoveState)
            eventInfo.idToResizeMoveState = {};
    ...
    var _flushResizeMoveActions = function (id, idToResizeMoveState) {
        ...
        while (moveInfo || resizeInfo || rotateInfo) {
            ...
            if (resizeInfo) {
                ...
                if (rotateInfo)
                    duration = Math.min(duration, rotateInfo.options.duration - rotate.used);
                ...
                if (moveInfo)
                    options.moveStop = { start: move.used, end: move.used + duration, len: moveInfo.options.duration };
                if (rotateInfo)
                    options.rotateStop = { start: rotate.used, end: rotate.used + duration, len: rotateInfo.options.duration };
                _addResize(id, eventInfo, resizeInfo, options, moveInfo, rotateInfo);
    ...
    _actionHandlers.setAdaptiveView = function (eventInfo, actions, index) {
        ...
        if (view)
            $ax.adaptive.setAdaptiveView(view);
    };
    ...
    var _includeVars = function (target, eventInfo) {
        ...
        if (target.urlLiteral) {
            ...
            if (literal.exprType == 'stringLiteral' && literal.value.indexOf('[[') == 0 && literal.value.indexOf(']]' == literal.value.length - 2) && literal.stos.length == 1 && sto.sto == 'item' && eventInfo.item) {
                ...
                if (data && $ax.public.fn.IsPage(data.type))
                    return true;
            }
        }
        return false;
    ...
    var linkFrame = function (eventInfo, action) {
        for (var i = 0; i < action.framesToTargets.length; i++) {
            ...
            if (id)
                $ax('#' + $ax.INPUT(id)).openLink(url, includeVars);
        }
    };
    var _repeatPanelMap = {};
                    ...
                    _addAnimation(elementId, queueTypes.setState, function () {
                        ...
                        if (stateInfo.setStateType == "value") {
                            ...
                            if (!stateNameFound) {
                                ...
                                if (isNaN(stateNumber) || stateNumber <= 0 || stateNumber > panelCount || Math.round(stateNumber) != stateNumber)
                            ...
                            if (typeof (repeat) == 'number')
                                _repeatPanelMap[elementId] = info;
    ...
    var _progessPanelState = function (id, stateInfo, eventInfo, skipFirst) {
        ...
        if (currentStateId != '') {
            ...
            if (direction == "next") {
                ...
                if (stateNumber <= 0) {
                    if (loop)
                        stateNumber = $ax.visibility.GetPanelStateCount(id);
                    else {
                        delete _repeatPanelMap[id];
                        return _fireAnimationFromQueue(id, queueTypes.setState);
                    }
            ...
            if (hasRepeat && _repeatPanelMap[id] != stateInfo)
                return _fireAnimationFromQueue(id, queueTypes.setState);
            if (!skipFirst)
                $ax('#' + id).SetPanelState(stateNumber, options, eventInfo, stateInfo.showWhenSet);
            else
                _fireAnimationFromQueue(id, queueTypes.setState);
            if (hasRepeat) {
                var animate = options && options.animateIn;
                if (animate && animate.easing && animate.easing != 'none' && animate.duration > repeat)
                ...
                window.setTimeout(function () {
                    ...
                    if (_repeatPanelMap[id] != stateInfo)
                        return;
    ...
    var _addMove = function (elementId, eventInfo, moveInfo, optionsOverride) {
        ...
        if ($ax.public.fn.IsLayer($obj(elementId).type)) {
            ...
            _addAnimation(elementId, queueTypes.move, function () {
                ...
                //                $ax.event.raiseSyntheticEvent(elementId, "onMove");
                if (loc.moveTo)
                    $ax('#' + elementId).moveTo(loc.x, loc.y, options);
                else
                    $ax('#' + elementId).moveBy(loc.x, loc.y, options);
            });
        }
    };
                    ...
                    if (moveWithThis) {
                        ...
                        if (stoScopes) {
                            for (var s in stoScopes) {
                                ...
                                if (!boundaryScope.localVariables)
                                    continue;
    ...
    var _updateLeftExprVariable = function (exprTree, xValue, yValue) {
        if (exprTree.leftExpr && !exprTree.leftExpr.op) {
            ...
            if (localVars) {
                if (localVars.withx)
                    localVars.withx.value = xValue;
                if (localVars.withy)
                    localVars.withy.value = yValue;
            }
        }
        //traversal
        if (exprTree.op) {
    ...
    var _addRotate = function (elementId, eventInfo, rotateInfo, options, moveInfo) {
        ...
        var eval = function (boundingRect) {
            ...
            if (!rotateInfo.options.clockwise)
                    ...
                    animations.push({
                        ...
                        func: function () {
                            ...
                            if (isLayer)
                                ...
                                $ax('#' + id).circularMoveAndRotate(degreeDelta, options, centerPoint.x, centerPoint.y, rotate, moveDelta);
                        }
                    });
                    if (!isLayer)
            ...
            animations.push({
                ...
                func: function () {
                    ...
                    if (offset.x == 0 && offset.y == 0)
                        _rotateSingle(elementId, rotateDegree, rotateInfo.rotateType == 'location', delta, options, options.stop, true);
                    else
    ...
    var _updateOffset = function (offset, anchor, boundingRect) {
        if (anchor.indexOf('left') != -1)
            offset.x -= boundingRect.width / 2;
        if (anchor.indexOf('right') != -1)
            offset.x += boundingRect.width / 2;
        if (anchor.indexOf('top') != -1)
            offset.y -= boundingRect.height / 2;
    ...
    var _initRotateLayer = function (elementId, rotateInfo, rotateDegree, options, stop) {
        ...
        if (layerDegree === undefined)
            layerDegree = 0;
        else
            layerDegree = parseFloat(layerDegree);
    ...
    var _applyAnchorToResizeOffset = function (deltaWidth, deltaHeight, anchor) {
        var offset = {};
        if (anchor.indexOf('left') != -1)
            offset.x = -deltaWidth / 2;
        else if (anchor.indexOf('right') != -1)
            offset.x = deltaWidth / 2;
        if (anchor.indexOf('top') != -1)
            offset.y = -deltaHeight / 2;
    ...
    var _getSizeFromInfo = function (resizeInfo, eventInfo, oldWidth, oldHeight, targetElement) {
        ...
        if (state && state.resizeResult)
            return state.resizeResult;
    ...
    var _getCssForResizingWidget = function (elementId, eventInfo, anchor, newWidth, newHeight, oldWidth, oldHeight, delta, stop, handleMove) {
        ...
        if ($ax.dynamicPanelManager.isPercentWidthPanel(obj))
            var is100Dp = true;
        else
            css.width = oldWidth + deltaWidth;
        var jobj = $jobj(elementId);
        ...
        if (obj.fixedVertical) {
            if (obj.fixedVertical == 'middle')
                anchor = obj.fixedHorizontal;
            else
                anchor = obj.fixedVertical + (obj.fixedHorizontal == 'center' ? '' : ' ' + obj.fixedHorizontal);
        }
    ...
    var _getCssForResizingLayerChild = function (elementId, anchor, layerBoundingRect, widthChangedPercent, heightChangedPercent, deltaLoc) {
        ...
        if (anchor.indexOf("top") > -1) {
            ...
            if (resizable)
                ...
                if (centerTopChange > 0)
                    topDelta = centerTopChange + Math.abs(currentSize.height * heightChangedPercent / 2);
            ...
            if (anchor.indexOf("left") > -1) {
                ...
                if (resizable)
                    ...
                    if (centerLeftChange > 0)
                        leftDelta = centerLeftChange + Math.abs(currentSize.width * widthChangedPercent / 2);
        ...
        if (topDelta)
            deltaLoc.y += topDelta;
        if (leftDelta && changeLeft)
            deltaLoc.x += leftDelta;
        return css;
    };
    ...
    _actionHandlers.modifyDataSetEditItems = function (eventInfo, actions, index) {
        ...
        for (var i = 0; i < repeaters.length; i++) {
            ...
            if (data.type == 'this') {
                ...
                if (add)
                    $ax.repeater.addEditItems(repeaterId, [itemId]);
                else
                    $ax.repeater.removeEditItems(repeaterId, [itemId]);
            }
            else if (data.type == 'all') {
                var allItems = $ax.repeater.getAllItemIds(id);
                if (add)
                    $ax.repeater.addEditItems(id, allItems);
                else
                    $ax.repeater.removeEditItems(id, allItems);
            }
            else {
                var oldTarget = eventInfo.targetElement;
                ...
                if (add)
                    $ax.repeater.addEditItems(id, itemIdsToAdd);
                else
                    $ax.repeater.removeEditItems(id, itemIdsToAdd);
            }
        }
        _dispatchAction(eventInfo, actions, index + 1);
    };
    ...
    _actionHandlers.addFilterToRepeater = function (eventInfo, actions, index) {
        ...
        for (var i = 0; i < action.repeatersToAddFilter.length; i++) {
            ...
            if (!id || _ignoreAction(id))
                continue;
            $ax.repeater.addFilter(id, addFilterInfo.removeOtherFilters, addFilterInfo.label, addFilterInfo.filter, addFilterInfo.condition, addFilterInfo.columnName, addFilterInfo.comboType, eventInfo.srcElement);
            _addRefresh(id);
        }
        _dispatchAction(eventInfo, actions, index + 1);
    };
    _actionHandlers.removeFilterFromRepeater = function (eventInfo, actions, index) {
        ...
        for (var i = 0; i < action.repeatersToRemoveFilter.length; i++) {
            ...
            if (!id || _ignoreAction(id))
                continue;
            if (removeFilterInfo.removeAll)
                $ax.repeater.removeFilter(id);
    ...
    _actionHandlers.removeSortFromRepeater = function (eventInfo, actions, index) {
        ...
        for (var i = 0; i < action.repeatersToRemoveSort.length; i++) {
            ...
            if (!id || _ignoreAction(id))
                continue;
            if (removeSortInfo.removeAll)
                $ax.repeater.removeSort(id);
    ...
    _actionHandlers.setItemsPerRepeaterPage = function (eventInfo, actions, index) {
        ...
        for (var i = 0; i < action.repeatersToSetItemCount.length; i++) {
            ...
            if (!id || _ignoreAction(id))
                continue;
            if (setItemCountInfo.noLimit)
                $ax.repeater.setNoItemLimit(id);
            else
                $ax.repeater.setItemLimit(id, setItemCountInfo.itemCountValue, eventInfo);
            _addRefresh(id);
        }
        _dispatchAction(eventInfo, actions, index + 1);
    };
    _actionHandlers.refreshRepeater = function (eventInfo, actions, index) {
        ...
        for (var i = 0; i < action.repeatersToRefresh.length; i++) {
            ...
            if (id)
                _tryRefreshRepeater(id, eventInfo);
        }
        _dispatchAction(eventInfo, actions, index + 1);
    };
    ...
    _action.tryRefreshRepeaters = function (ids, eventInfo) {
        for (var i = 0; i < ids.length; i++)
            _tryRefreshRepeater(ids[i], eventInfo);
    };
    _actionHandlers.scrollToWidget = function (eventInfo, actions, index) {
        ...
        if (elementIds.length > 0)
            $ax('#' + elementIds[0]).scroll(action.options);
        _dispatchAction(eventInfo, actions, index + 1);
    };
    _actionHandlers.enableDisableWidgets = function (eventInfo, actions, index) {
        ...
        for (var i = 0; i < action.pathToInfo.length; i++) {
            ...
            var enableAction = action.pathToInfo[i].enableDisableInfo.enableAction;
            var isToggle = enableAction === 'Toggle';
            var isEnabled = enableAction === 'Enable';
            for (var j = 0; j < elementIds.length; j++) {
                var elementId = elementIds[j];
                var toggleValue = $ax.style.IsWidgetDisabled(elementId);
    ...
    _actionHandlers.setImage = function (eventInfo, actions, index) {
        ...
        for (var i = 0; i < action.imagesToSet.length; i++) {
            ...
            if (view && imgInfo.adaptive[view])
                imgInfo = imgInfo.adaptive[view];
            else
                imgInfo = imgInfo.base;
            ...
            if (elementIds.length > 0) {
                ...
                if (action.selectText && !$ax.placeholderManager.isActive(elementIds[0])) {
                    ...
                    for (var i = 0; i < elementChildren.length; i++) {
                        if (elementChildren[i].id.indexOf('_input') == -1)
                            continue;
            ...
            for (var j = 0; j < targetObjIds.length; j++) {
                ...
                if (firedEventNames) {
                    for (k = 0; k < firedEventNames.length; k++) {
                        ...
                        if (event)
                            $ax.event.handleEvent(isPage ? '' : targetObjId, eventInfo, event, false, true);
                    }
                }
                if (isPage)
