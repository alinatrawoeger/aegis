...
$axure.internal(function ($ax) {
    ...
    var _refreshRepeater = function (repeaterId, eventInfo, refreshType, itemsPregen) {
        if (!refreshType)
            refreshType = _refreshType.reset; // Set default
        if (!repeatersReady) {
        ...
        if (itemsPregen) {
            ...
            orderedIds = getOrderedIds(repeaterId, eventInfo);
        var ids = [];
        ...
        if (itemsPregen) {
            ...
            for (var pos = start; pos < end; pos++) {
                ...
                if (jobj.hasClass('preeval'))
                    refreshType = _refreshType.preEval;
                for (var i = 0; i < templateIds.length; i++) {
                    var elementId = _createElementId(templateIds[i], itemId);
                    $ax.initializeObjectEvents($ax('#' + elementId), refreshType);
                    if (refreshType != _refreshType.preEval || elementId == itemElementId)
                        continue;
                    var elJobj = $jobj(elementId);
            ...
            var altDiv = div;
            if (hasAltColor)
                altDiv = _applyColorCss(altColor, div.clone());
            ...
            for (pos = start; pos < end; pos++) {
                ...
                if (obj.repeaterPropMap.isolateRadio) {
                    ...
                    for (var radioIndex = 0; radioIndex < radioButtons.length; radioIndex++) {
                        ...
                        if (oldName)
                            radio.attr('name', oldName + '-' + itemId);
                    }
                }
                copy.css({
                    'position': 'absolute',
        ...
        if (viewId)
            $ax.adaptive.applyView(viewId, query);
        else
            $ax.visibility.resetLimboAndHiddenToDefaults(_getItemQuery(repeaterId, preevalMap));
        $ax.annotation.createFootnotes(query, true);
        for (var index = 0; index < ids.length; index++) {
            id = ids[index];
            if ($ax.ieColorManager)
                $ax.ieColorManager.applyBackground($ax('#' + id));
        ...
        for (pos = start; pos < end; pos++) {
            ...
            if (!preevalMap[orderedIds[pos]]) {
                ...
                var wasTracing = $ax.messageCenter.getState('isTracing') === true;
                ...
                if (wasTracing)
                    $ax.messageCenter.setState('isTracing', wasTracing);
    ...
    _repeaterManager.refreshAllRepeaters = function () {
        $ax('*').each(function (diagramObject, elementId) {
            if (!$ax.public.fn.IsRepeater(diagramObject.type))
                return;
            if ($ax.visibility.isElementIdLimboOrInLimboContainer(elementId))
                return;
    ...
    _repeaterManager.refreshRepeaters = function (ids, eventInfo) {
        for (var i = 0; i < ids.length; i++)
            _refreshRepeater(ids[i], eventInfo);
    };
    ...
    _repeaterManager.setDisplayProps = function (obj, repeaterId, itemIndex) {
        ...
        for (var i = 0; i < count; i++) {
            if (data[start + i].index == itemIndex)
                index = i + 1;
        }
        if (index == -1)
            return;
    ...
    var SORT_TYPE_NUMERIC = 0x1;
    var SORT_TYPE_ALPHA = 0x2;
    var SORT_TYPE_ALPHA_CASE = 0x3;
    var SORT_TYPE_ALPHANUMERIC = 0x4;
    var SORT_TYPE_DATE1 = 0x10;
    var SORT_TYPE_DATE2 = 0x11;
    var getOrderedIds = function (repeaterId, eventInfo) {
        ...
        if (sorts.length != 0 && data.length > 1) {
            ...
            var mergesort = function (list, start, end, compare) {
                ...
                if (middle - start > 1)
                    mergesort(list, start, middle, compare);
                if (end - middle > 1)
                    mergesort(list, middle, end, compare);
                var index1 = start;
                var index2 = middle;
                var tempList = [];
                ...
                while (index1 < middle)
                    tempList[tempList.length] = list[index1++];
                while (index2 < end)
                    tempList[tempList.length] = list[index2++];
                ...
                return function (row1, row2) {
                    ...
                    if (type == SORT_TYPE_ALPHANUMERIC) {
                        ...
                        if (!date1.valid && !date2.valid)
                            return 0;
                        if (!date1.valid || !date2.valid)
                            return date1.valid ? -1 : 1;
                        var diff = date2.year - date1.year;
                        if (diff == 0)
                            diff = date2.month - date1.month;
                        if (diff == 0)
            ...
            for (var i = 0; i < sorts.length; i++)
                compareFunc = getComparator(sorts[i].columnName, sorts[i].ascending, sorts[i].sortType, compareFunc);
    ...
    var SMART_FILTER_COMBO = 0x0, ALL_FILTER_COMBO = 0x1, ANY_FILTER_COMBO = 0x2;
    var applyFilter = function (repeaterId, data, eventInfo) {
        ...
        if (filters.length < 1)
            return;
        if (!eventInfo)
            eventInfo = $ax.getBasicEventInfo();
        ...
        var filterType = filters[filters.length - 1].comboType;
        var filtersByColumnData = SMART_FILTER_COMBO == filterType ? _filtersByColumn(filters) : {};
        var idToWidgetInfoCache = {};
        for (var itemIndex = 1; itemIndex <= data.length; itemIndex++) {
            if (!_passedFilters(repeaterId, filters, eventInfo, idToWidgetInfoCache, itemIndex, filterType, filtersByColumnData))
                continue;
            dataFiltered[dataFiltered.length] = data[itemIndex - 1];
        }
        for (i = 0; i < dataFiltered.length; i++)
            data[i] = dataFiltered[i];
        while (data.length > dataFiltered.length)
            data.pop();
    ...
    var _filterAll = function (repeaterId, filters, eventInfo, idToWidgetInfo, itemIndex) {
        for (var i = 0; i < filters.length; i++) {
            ...
            if (!_evaluateFilter(filter, eventInfo))
                return false;
        }
        return true;
    };
    ...
    var _filterSmart = function (repeaterId, filtersByColumnData, eventInfo, idToWidgetInfo, itemIndex) {
        var columnNames = filtersByColumnData.columnNames;
        var columnNamesToFilters = filtersByColumnData.columnNamesToFilters;
        var expressionFilters = filtersByColumnData.expressionFilters;
        for (var i = 0; i < columnNames.length; ++i) {
            var filtersForColumn = columnNamesToFilters[columnNames[i]];
            if (!_filterAny(repeaterId, filtersForColumn, eventInfo, idToWidgetInfo, itemIndex))
                return false;
        }
    ...
    var _evaluateFilter = function (filter, eventInfo) {
        var result = $ax.expr.evaluateExpr(filter.filter ? filter.filter : filter.condition, eventInfo);
        if (result === true)
            return true;
        return result == 'true';
    };
    var _updateEventInfoForFilter = function (repeaterId, filter, eventInfo, idToWidgetInfo, itemIndex) {
        ...
        if (!idToWidgetInfo[eventInfo.srcElement])
            idToWidgetInfo[eventInfo.srcElement] = $ax.getWidgetInfo(eventInfo.srcElement);
    ...
    var _addFilter = function (repeaterId, removeOtherFilters, label, filter, condition, columnName, comboType, thisId) {
        if (removeOtherFilters)
            _removeFilter(repeaterId);
        var filterList = repeaterToFilters[repeaterId];
        if (!filterList)
            repeaterToFilters[repeaterId] = filterList = [];
        var filterObj = { thisId: thisId };
        if (label)
    ...
    var _addSort = function (repeaterId, label, columnName, ascending, toggle, sortType, removeAllSorts) {
        ...
        if (label)
            sortObj.label = label;
        if (removeAllSorts)
            repeaterToSorts[repeaterId] = [sortObj];
        else
    ...
    var _removeSort = function (repeaterId, label) {
        ...
        for (var i = sortList.length - 1; i >= 0; i--) {
            ...
            if (sortObj.label && sortObj.label == label)
                $ax.splice(sortList, i, 1);
        }
    };
    ...
    var _setRepeaterToPage = function (repeaterId, type, value, eventInfo) {
        ...
        if (!dataSet)
            dataSet = repeaterToCurrentDataSet[repeaterId];
    ...
    _repeaterManager.setItemSize = function (repeaterId, itemId, width, height) {
        ...
        if (deltaX)
            _pushItems(repeaterId, itemId, deltaX, false, true);
        if (deltaY)
            _pushItems(repeaterId, itemId, deltaY, true, true);
        if (deltaX || deltaY)
    ...
    var _pushItems = _repeaterManager.pushItems = function (repeaterId, itemId, delta, vertical, suppressFire) {
        ...
        if (!wrap && vert != vertical) {
            ...
            if (sizes.length != 1) {
                if (after >= limit)
                    repeaterDelta = after - Math.max(limit, before);
                else if (before > limit)
                    repeaterDelta = limit - before;
                else
                    repeaterDelta = 0;
            }
        ...
        if (wrap) {
            outer: for (; index < sizes.length; index++) {
                ...
                for (index2 = 0; index2 < innerSizes.length; index2++)
                    if (innerSizes[index2].itemId == itemId)
                        break outer;
            }
        }
        else {
            for (; index < sizes.length; index++)
                if (sizes[index].itemId == itemId)
                    break;
        }
        ...
        if (vert == vertical) {
            ...
            for (i++; i < array.length; i++)
                itemIdsEffected.push(array[i].itemId);
        }
        else {
    ...
    var _updateRepeaterSize = function (prop, jobj, delta, vert) {
        ...
        var border = 0;
        if (vert)
            border += $ax.getNumFromPx(jobj.css('border-top-width')) + $ax.getNumFromPx(jobj.css('border-bottom-width'));
        else
            border += $ax.getNumFromPx(jobj.css('border-left-width')) + $ax.getNumFromPx(jobj.css('border-right-width'));
        val += border;
        jobj.css(prop, val);
    ...
    var _deleteItemsFromDataSet = function (repeaterId, eventInfo, type, rule) {
        ...
        if (type == 'this')
            items = [_getItemIdFromElementId(eventInfo.srcElement)];
        else if (type == 'marked')
            ...
            for (var i = 0; i < visibleData.length + deleteDataMap.deletedIds.length; i++) {
                var index = i + 1;
                if (deleteDataMap.deletedIds.indexOf(index) != -1)
                    continue;
        ...
        for (i = 0; i < items.length; i++) {
            ...
            if (deleteDataMap.deletedIds.indexOf(itemId) != -1)
                continue;
            ...
            for (var j = editItems.length - 1; j >= 0; j--) {
                ...
                if (editItem == itemId)
                    $ax.splice(editItems, j, 1);
                else if (editItem > itemId)
                    editItems[j] = editItem - 1;
            }
        }
    };
    ...
    var _updateEditItemsInDataSet = function (repeaterId, propMap, eventInfo, type, rule) {
        ...
        if (type == 'this')
            ...
            items = repeaterToEditItems[repeaterId];
        else {
            // This should be rule
        ...
        for (var prop in propMap) {
            ...
            for (i = 0; i < items.length; i++) {
                ...
                if (data.type == 'literal') {
                    ...
                    if (typeof (data) == 'object' && data.isWidget)
                        data = data.text;
    ...
    var _removeSuffixFromElementId = function (id) {
        ...
        if (suffixId != -1)
            return id.substr(0, suffixId);
        var partId = id.indexOf('p');
        if (partId != -1)
            return _createElementId(id.substr(0, partId), _getItemIdFromElementId(id)); // item id is after part, but before suffix
        return id;
    };
...
$axure.internal(function ($ax) {
    ...
    var _fitParentPanel = function (widgetId) {
        ...
        if (parentLayer) {
            if (_updateLayerRectCache(parentLayer))
                _fitParentPanel(parentLayer);
            return;
        }
    ...
    var _updateMobileScroll = _dynamicPanelManager.updateMobileScroll = function (panelId, stateId, blockResetScroll) {
        ...
        if ($ax.dynamicPanelManager.isIdFitToContent(panelId))
            return false;
        var obj = $obj(panelId);
        if (!obj || obj.scrollbars.toLowerCase() == 'none')
            return false;
    ...
    _dynamicPanelManager.updatePanelContentPercentWidth = function (elementId) {
        ...
        stateChildrenQuery.children('.panel_state_content').each(function () {
            $(this).children('.ax_dynamic_panel').each(function () { _updatePanelPercentWidth(this.id); });
        });
    };
    ...
    var _updatePercentPanelToLeftCache = function (obj, elementId, overwrite) {
        ...
        if (percentPanelToLeftCache[elementId] == undefined || overwrite) {
            if (obj.fixedHorizontal == 'center')
                percentPanelToLeftCache[elementId] = $ax.getNumFromPx(jObj.css('margin-left'));
            else if (obj.fixedHorizontal == 'right')
                percentPanelToLeftCache[elementId] = axObj.width() + $ax.getNumFromPx(jObj.css('right'));
            else
    ...
    var _updatePanelPercentWidth = _dynamicPanelManager.updatePanelPercentWidth = function (elementId) {
        ...
        while (stateParent.children()[0].id.indexOf($ax.visibility.CONTAINER_SUFFIX) != -1)
        ...
        if (obj.fixedHorizontal == 'center')
            ...
            stateChildrenQuery.children('.panel_state_content').css('margin-left', panelLeft - x + 'px');
    };
    _dynamicPanelManager.updateParentsOfNonDefaultFitPanels = function () {
        $ax('*').each(function (diagramObject, elementId) {
            if (!$ax.public.fn.IsDynamicPanel(diagramObject.type) || !diagramObject.fitToContent)
                return;
            if ($ax.visibility.isElementIdLimboOrInLimboContainer(elementId))
                return;
    ...
    var _getShownStateId = function (id) {
        var obj = $obj(id);
        if (!obj || !$ax.public.fn.IsDynamicPanel(obj.type))
            return id;
    ...
    var _updateFitPanel = function (panelId, stateIndex) {
        ...
        $jobj(stateContentId).children('.ax_dynamic_panel').each(function () { _updatePanelPercentWidth(this.id); });
    ...
    var getParentPanel = function (widgetId, path, targetId) {
        ...
        if (parentMasterItemId != widgetItemId)
            return undefined;
    ...
    var getParentLayer = function (widgetId, path) {
        ...
        if (widgetParentPanel) {
            ...
            if (!parentMasterParentPanel || widgetParentPanel.parent != parentMasterParentPanel.parent)
                return undefined;
        }
        //check if the master is in the same repeater as the widgetId widget
        var parentMasterItemId = $ax.repeater.getItemIdFromElementId(parentMaster);
    ...
    _dynamicPanelManager.getFixedInfo = function (panelId) {
        var panelObj = $obj(panelId);
        if (!panelObj || !$ax.public.fn.IsDynamicPanel(panelObj.type))
            return {};
        ...
        if (info.horizontal == 'left')
            info.x = $ax.getNumFromPx(jobj.css('left'));
        ...
        if (info.vertical == 'top')
            info.y = $ax.getNumFromPx(jobj.css('top'));
    ...
    var _compressToggle = function (id, vert, show, easing, duration, delta) {
        if (delta === void 0) { delta = NaN; }
    ...
    var _compressDelta = function (id, oldState, newState, vert, easing, duration, delta) {
        if (delta === void 0) { delta = NaN; }
        var oldQuery = $jobj(oldState);
        var newQuery = $jobj(newState);
    ...
    var _compressMove = function (id, vert, isResize, targetRect, easing, duration, delta, complimentaryDelta) {
        if (delta === void 0) { delta = NaN; }
        if (complimentaryDelta === void 0) { complimentaryDelta = 0; }
        ...
        if (magSign < 0)
            threshold = vert ? targetRect.bottom : targetRect.right;
        if (isNaN(delta)) {
            delta = magnitude;
        }
        else {
    ...
    var _compress = function (id, vert, threshold, delta, easing, duration, clampWidth, complimentaryDelta) {
        if (complimentaryDelta === void 0) { complimentaryDelta = 0; }
        ...
        if (isNaN(clamp.start) || isNaN(clamp.end) || isNaN(threshold) || isNaN(delta))
            return;
        ...
        if (0 != parentScrollLeft)
            scrollParentObj.scrollLeft(parentScrollLeft);
        if (0 != parentScrollTop)
            scrollParentObj.scrollTop(parentScrollTop);
        if (windowObj.scrollLeft() != scrollLeft)
            windowObj.scrollLeft(scrollLeft);
        if (windowObj.scrollTop() != scrollTop)
            windowObj.scrollTop(scrollTop);
        if (layer)
            $ax.visibility.popContainer(layer, false);
        // Do item push
        ...
        if (parent != $ax.repeater.createElementId(repeaterId, itemId))
            return;
    ...
    var _compressChildrenHelper = function (id, children, vert, threshold, delta, clamp, easing, duration, parentLayer, complimentaryDelta) {
        if (complimentaryDelta === void 0) { complimentaryDelta = 0; }
        var toMove = [];
        var allMove = true;
        if (isNaN(complimentaryDelta))
        ...
        for (var i = 0; i < children.length; i++) {
            ...
            if ($ax.getTypeFromElementId(childId) == $ax.constants.LAYER_TYPE) {
                ...
                if ($ax.getLayerParentFromElementId(id) != childId && _objectNeedsCompress(childId, vert, threshold, clamp, parentLayer))
                    addSelf = true;
                else
            ...
            if (numbers.length < 1 || isNaN(Number(numbers[0])) || (numbers.length == 2 && isNaN(Number(numbers[1]))) || numbers.length > 2)
                continue;
    ...
    var propagate = function (id, hover, value) {
        var hoverChildren = function (children) {
            ...
            for (var i = 0; i < children.length; i++) {
                ...
                if (obj == null)
                    continue;
                if (($ax.public.fn.IsDynamicPanel(obj.type) || $ax.public.fn.IsLayer(obj.type) || $ax.public.fn.IsRepeater(obj.type)) && !obj.propagate)
                    continue;
                if (hover)
                    $ax.style.SetWidgetHover(elementId, value);
                else
                    $ax.style.SetWidgetMouseDown(elementId, value);
