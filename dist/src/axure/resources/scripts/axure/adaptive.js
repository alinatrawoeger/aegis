$axure.internal(function ($ax) {
    ...
    $ax.adaptive.loadFinished = function () {
        if (_loadFinished)
            return;
        _loadFinished = true;
        if ($ax.adaptive.currentViewId)
            $ax.viewChangePageAndMasters();
        else
            $ax.postAdaptiveViewChanged();
    };
    var _handleResize = function (forceSwitchTo) {
        if (!_auto)
            return;
        if (_auto && _autoIsHandledBySidebar && !forceSwitchTo)
            return;
        var $window = $(window);
    ...
    var _switchView = function (viewId, forceSwitchTo) {
        ...
        if (viewId == previousViewId) {
            if (forceSwitchTo)
                $ax.postAdaptiveViewChanged(forceSwitchTo);
            return;
        }
        $ax('*').each(function (obj, elementId) {
            if (!$ax.public.fn.IsTreeNodeObject(obj.type))
                return;
        ...
        $axure('*').each(function (diagramObject, elementId) {
            if (diagramObject.isContained)
                return;
            if ($ax.getParentRepeaterFromElementIdExcludeSelf(elementId))
                return;
        ...
        if (!viewId) {
            $axure('*').each(function (diagramObject, elementId) {
                ...
                if (child.length)
                    $ax.style.transformTextWithVerticalAlignment(child[0].id, function () { });
            });
    ...
    var _getImageWithTag = function (image, tag) {
        ...
        for (var component in image) {
            ...
            if (componentImage)
                flattened[component] = componentImage;
        }
        return flattened;
    };
    ...
    var _applyAdaptiveViewOnObject = function (diagramObject, elementId, viewIdChain, viewId, limboIds, hiddenIds) {
        ...
        if (images) {
            if (diagramObject.type == 'horizontalLine' || diagramObject.type == 'verticalLine') {
                ...
                if (compoundUrl)
                    $ax.style.applyImage(elementId, compoundUrl, state);
            }
            else {
        ...
        if (compoundStyle.visible === false)
            hiddenIds[scriptId] = true;
    };
    var _matchImage = function (id, images, viewIdChain, state, doNotDecomposeState) {
        var overrideState = $ax.style.highestPriorityBaseState(state);
        var override = $ax.style.getElementImageOverride(id, overrideState);
        if (override)
            return override;
        if (!images)
            return undefined;
        ...
        for (var i = viewIdChain.length - 1; i >= 0; i--) {
            var viewId = viewIdChain[i];
            var img = findImage(images, scriptId, state, viewId);
            if (img)
                return img;
        }
        // check for the default state style
        var defaultStateImage = findImage(images, scriptId, state);
        if (defaultStateImage)
            return defaultStateImage;
        if (doNotDecomposeState)
            return undefined;
        state = $ax.style.decomposeState(state);
        if (state)
            return _matchImage(scriptId, images, viewIdChain, state);
        // SHOULD NOT REACH HERE! NORMAL SHOULD ALWAYS CATCH AT THE DEFAULT!
        return images['normal~']; // this is the default
    };
    var findImage = function (images, scriptId, state, viewId) {
        if (!images)
            return undefined;
        if (!viewId)
            viewId = "";
        var withScript = scriptId + "~" + state + "~" + viewId;
        var img = images[withScript];
        if (!img)
            img = images[state + "~" + viewId];
        return img;
    };
    ...
    $ax.adaptive.getImageForStateAndView = function (id, state) {
        ...
        if (diagramObject.generateCompound)
            ...
            return _matchImage(id, diagramObject.images, viewIdChain, state);
    };
    var _getAdaptiveView = function (winWidth, winHeight) {
        var _isViewOneGreaterThanTwo = function (view1, view2, winHeight) {
            ...
            if (view1.size.width == view2.size.width) {
                if (view2.size.height <= winHeight)
                    return view1.size.height > view2.size.height && view1.size.height <= winHeight;
                else
                    return view1.size.height < view2.size.height;
            }
            return false;
        };
        ...
        for (var i = 0; i < _enabledViews.length; i++) {
            ...
            if (_isWindowWidthLessThanViewWidth(view, winWidth, winHeight)) {
                if (!less || _isViewOneLessThanTwo(view, less))
                    less = view;
            }
        }
        return greater || less;
        ...
        if (message == 'switchAdaptiveView') {
            ...
            if (data.scale != 0 && data.scale != 3) {
                ...
                if (data.scale == 2) {
                    ...
                    if (isCentered)
                        contentOriginOffset = scaleN * (bodyWidth / 2);
                }
                ...
                if (data.scale == 3) {
                    var zoom = $(document).find("html").attr("zoom") ? parseInt($(document).find("html").attr("zoom"), 10) : 100;
    ...
    var _removeNiceScroll = $ax.adaptive.removeNiceScroll = function ($container, blockResetScroll) {
        ...
        if (IE)
            $container.css({ '-ms-overflow-y': '', 'overflow-y': '', '-ms-overflow-style': '', '-ms-touch-action': '' });
        if (!emulateTouch)
            return;
    ...
    var _addNiceScroll = $ax.adaptive.addNiceScroll = function ($container, options, blockResetScroll) {
        ...
        if (options.emulatetouch)
            _addNiceScrollExitDetector($container);
        //clean up nicescroll css so child scroll containers show scrollbars in IE
        if (IE)
            $container.css({ '-ms-overflow-y': '', '-ms-overflow-style': '' });
        if (IOS)
    ...
    $ax.adaptive.getNiceScrollContainer = function (element) {
        var parent = element;
        while (parent) {
            if ($(parent).getNiceScroll().length > 0)
                return parent;
    ...
    $ax.adaptive.setAdaptiveView = function (view) {
        ...
        if (!_isAdaptiveInitialized()) {
            ...
            _handleLoadViewId(viewIdForSitemapToUnderstand);
    };
    $ax.adaptive.initialize = function () {
        ...
        if (_views && _views.length > 0) {
            for (var i = 0; i < _views.length; i++) {
                ...
                if (useViews)
                    _enabledViews[_enabledViews.length] = view;
            }
            if (_autoIsHandledBySidebar && _initialViewSizeToLoad)
                _handleSetViewForSize(_initialViewSizeToLoad.width, _initialViewSizeToLoad.height);
            else
