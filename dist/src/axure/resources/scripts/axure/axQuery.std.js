...
$axure.internal(function ($ax) {
    ...
    $ax.public.fn.SetPanelState = function (stateNumber, options, eventInfo, showWhenSet) {
        ...
        for (var index = 0; index < elementIds.length; index++) {
            ...
            if ($ax.public.fn.IsDynamicPanel($ax.getTypeFromElementId(elementId))) {
                ...
                $ax.visibility.SetPanelState(elementId, newStateName, animateOutInfo.easingType, animateOutInfo.direction, animateOutInfo.duration, animateInInfo.easingType, animateInInfo.direction, animateInInfo.duration, showWhenSet);
        ...
        for (var index = 0; index < elementIds.length; index++) {
            ...
            if (options && options.showType == 'lightbox') {
                ...
                if (lightbox.length == 0) {
                    ...
                    if (!fixedParentPanelId)
                        $('#base').append(lightbox);
                    else
                        $jobj(fixedParentPanelId).append(lightbox);
                    var wasVisible = $ax.visibility.IsIdVisible(elementId);
                ...
                var rects = {};
                if (src.valid)
                    rects.src = $ax.geometry.genRect(src, true);
                if (target.valid)
                    rects.target = $ax.geometry.genRect(target, true);
    ...
    var _setVisibility = function (elementId, value, options, useHide) {
        ...
        if (compress)
            $ax.dynamicPanelManager.compressToggle(elementId, options.vertical, value, options.compressEasing, options.compressDuration, (_a = options.compressDelta) !== null && _a !== void 0 ? _a : NaN);
        ...
        if (options && options.bringToFront)
            $ax.legacy.BringToFront(elementId);
    };
    $ax.public.fn.setOpacity = function (opacity, easing, duration) {
        ...
        for (var index = 0; index < elementIds.length; index++) {
            ...
            if (duration == 0 || easing == 'none') {
                ...
                query.animate({ opacity: opacity }, { duration: duration, easing: easing, queue: false, complete: onComplete });
        }
    };
    ...
    var _move = function (elementId, x, y, options, moveTo) {
        ...
        if ($ax.public.fn.IsLayer(obj.type)) {
            $ax.move.MoveWidget(elementId, x, y, options, moveTo, function () {
            ...
            $ax.move.MoveWidget(elementId, xDelta, yDelta, options, false, function () { $ax.dynamicPanelManager.fitParentPanel(elementId); }, true);
        }
    };
    ...
    $ax.public.fn.circularMoveAndRotate = function (degreeChange, options, centerPointLeft, centerPointTop, doRotation, moveDelta, resizeOffset, rotatableMove, moveComplete) {
        ...
        for (var index = 0; index < elementIds.length; index++) {
            ...
            if (doRotation)
                $ax.move.rotate(elementId, degreeChange, options.easing, options.duration, false, true, function () { $ax.dynamicPanelManager.fitParentPanel(elementId); });
            else
                $ax.action.fireAnimationFromQueue(elementId, $ax.action.queueTypes.rotate);
        }
    };
                ...
                if (query.css('position') == 'fixed' && ((obj.fixedHorizontal && obj.fixedHorizontal == 'center') || (obj.fixedVertical && obj.fixedVertical == 'middle'))) {
                    ...
                    if (loc) {
                        if (loc[0] != 0 && !$ax.dynamicPanelManager.isPercentWidthPanel(obj))
                            newLocationAndSizeCss['margin-left'] = '+=' + (Number(newLocationAndSizeCss['margin-left'].substr(2)) + loc[0]);
                        if (loc[1] != 0)
                            newLocationAndSizeCss['margin-top'] = '+=' + (Number(newLocationAndSizeCss['margin-top'].substr(2)) + loc[1]);
                    }
                }
                var onComplete = function () {
        ...
        if (arguments[0] == undefined) {
            ...
            for (var index = 0; index < elementIds.length; index++) {
                ...
                if ($ax.public.fn.IsTextBox(widgetType) || $ax.public.fn.IsTextArea(widgetType)) { //Do nothing for non rtf
                    ...
                    if ($(idRtf).length != 0)
                        SetWidgetRichText(idRtf, arguments[0]);
                }
            }
            return this;
        }
    ...
    var _scrollHelper = function (id, scrollX, scrollY, easing, duration) {
        ...
        if ($scrollable.is('body'))
            ...
            viewportLocation = $ax('#' + id).pageBoundingRect(true, $scrollable.attr('id')).location;
        ...
        if (!scrollX)
            targetLeft = scrollable.scrollLeft;
        if (!scrollY)
            targetTop = scrollable.scrollTop;
        if ($scrollable.is('body')) {
    ...
    $ax.public.fn.enabled = function () {
        if (arguments[0] == undefined) {
            ...
            if ($ax.public.fn.IsImageBox(widgetType) || $ax.public.fn.IsVector(widgetType)
                ...
                return this.jQuery().children(':disabled').length <= 0;
        }
        else {
            var elementIds = this.getElementIds();
            for (var index = 0; index < elementIds.length; index++) {
    ...
    var _boundingRectForIds = function (childIds) {
        ...
        for (var i = 0; i < childIds.length; i++) {
            ...
            if ($ax.visibility.limboIds[childId] ||
                ...
                $ax.public.fn.IsDynamicPanel(childObj.type) && childObj.fixedHorizontal)
                continue;
            var boundingRect = $ax('#' + childId).offsetBoundingRect();
            // Ignore empty groups
            if ($ax.public.fn.IsLayer(childObj.type) && boundingRect.width == 0 && boundingRect.height == 0)
                continue;
    ...
    $ax.public.fn.offsetBoundingRect = function (ignoreRotation) {
        ...
        if (movedLoc) {
            ...
            for (var i = 0; i < parents.length; i++) {
                ...
                else if (!$axure.fn.IsLayer(type))
                    break;
            }
        }
        else {
            if (!trap)
                trap = _displayWidget($ax.repeater.removeSuffixFromElementId(elementId));
        ...
        if (resizedSize) {
            ...
            if (oShadow.on) {
                if (oShadow.offsetX < 0)
                    size.width -= oShadow.offsetX;
                else
                    size.width += oShadow.offsetX;
                if (oShadow.offsetY < 0)
                    size.height -= oShadow.offsetY;
                else
                    size.height += oShadow.offsetY;
    ...
    $ax.public.fn.pageBoundingRect = function (ignoreRotation, scrollableId) {
        ...
        for (var i = 0; i < parentIds.length; i++) {
            ...
            //}
            var parentId = parentIds[i];
            if (parentId == parentScrollableId)
                break;
...
//# sourceMappingURL=axQuery.std.js.map