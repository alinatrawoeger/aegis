...
$axure.internal(function ($ax) {
    ...
    var _updateFlyout = function (panelId) {
        ...
        if (!$ax.geometry.polygonRegistered(label))
            return;
        ...
        if (!$ax.geometry.checkInsideRegion(label))
    ...
    var _registerFlyout = function (rects, panelId, srcId) {
        ...
        if (lastSrcId != srcId) {
            if (lastSrcId)
                $ax.style.RemoveRolloverOverride(lastSrcId);
            if (srcId) {
    ...
    var _reregisterAllFlyouts = function () {
        for (var panelId in panelToSrc)
            _reregisterFlyout(panelId);
    };
