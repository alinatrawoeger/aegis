...
(function () {
    ...
    function initializeEvents() {
        ...
        $maximizeContainer.mouseenter(function () { if (!MOBILE_DEVICE)
            $minimizeContainer.addClass('expandHovered'); });
        $maximizeContainer.mouseleave(function () { if (!MOBILE_DEVICE)
            $minimizeContainer.removeClass('expandHovered'); });
        ...
        if ($axure.mobileSupport.mobile) {
            ...
            $('#mainPanel').on('touchstart', (function (e) {
                ...
                if (now - lastTouch < 375) {
                    if (++touchCount === 3) {
                        if ($axure.player.isMobileMode() || MOBILE_DEVICE)
                            expand();
                        touchCount = 0;
                        e.preventDefault();
                    }
        ...
        $(window).on("orientationchange", function () {
            ...
            if (IOS && isMobileMode())
                setTimeout(function () { $axure.player.resizeContent(true); }, 250);
        });
    ...
    function initializeMainFrame() {
        ...
        if (SAFARI && document.location.href.indexOf('file://') >= 0) {
            $(mainFrame).on('load', function () {
                ...
                if (!canAccess)
                    window.location = 'resources/chrome/safari.html';
            });
        }
        if ($axure.player.settings != null && !$axure.player.settings.isExpo) {
            var linkUrlWithVars = $axure.getLinkUrlWithVars(getInitialUrl());
    ...
    function toggleSitemapMobileMode() {
        ...
        if (isMobileMode()) {
            ...
            if (MOBILE_DEVICE)
                $container.css('margin-left', '13px');
        }
        else {
    ...
    function zoomWheelHandler(event) {
        if (event.ctrlKey || event.metaKey) {
            ...
            var wheelEvent = event.originalEvent ? event.originalEvent : event;
            if (wheelEvent) {
                var deltaY = normalizeZoomWheelEvent(wheelEvent).deltaY;
                var timestamp = wheelEvent.timeStamp / 1e3;
                ...
                var zoomStep = deltaY < 0 ? zoom * 0.0409 + 0.0415 : zoom * 0.0394 + 0.0367;
                var newZoom = roundZoomValue(zoom + zoomStep * -deltaY);
                if ((zoom > 100 && newZoom <= 100) || (zoom < 100 && newZoom >= 100)) {
                    // enable stickiness
                    newZoom = 100;
                    disableChunkedZoomEvents = true;
                }
                var zoomPositionX = wheelEvent.clientX * zoom / 100 - zoom / 100;
                var zoomPositionY = wheelEvent.clientY * zoom / 100 - zoom / 100;
                var zoomPosition = { x: zoomPositionX, y: zoomPositionY };
    ...
    function zoomGestureChange(e) {
        ...
        gestureRaf = requestAnimationFrame(function () {
            ...
            var direction = Math.sign(e.scale - 1);
    ...
    function scrollWheelHandler(event) {
        if (!event.ctrlKey && !event.metaKey) {
            ...
            if (wheelEvent) {
                event.preventDefault();
                var scrollStep = isTrackPadDetected(event) ? 13 : 100;
                var deltaFactor = getDeltaWheelFactor();
                var deltaY = wheelEvent.deltaY ? -wheelEvent.deltaY * deltaFactor * scrollStep : 0;
                var deltaX = wheelEvent.deltaX ? -wheelEvent.deltaX * deltaFactor * scrollStep : 0;
    ...
    function common_contentDocument_onload() {
        (function bindHandlersWhenReady() {
            ...
            if ($($iframe.contentWindow.document.body).length === 0 || $iframe.contentWindow.document.URL === "about:blank") {
                ...
                var iframeWindow = $('#mainFrame')[0].contentWindow;
    ...
    var getPageSize = function () {
        var currentZoom = getCurrentZoom() / 100;
    ...
    var setPagePosition = $axure.player.setPagePosition = function (position) {
        ...
        var screenContainerLeft = (viewerSize.width / 2 > Math.abs(position.x) ? 0 : -position.x - viewerSize.width / 2);
        var screenContainerTop = -position.y - viewerSize.height / 2;
    ...
    var zoomIn = function (position) {
        var nextZoom = getNextStepOfZoom(1);
    ...
    var getNextStepOfZoom = function (offset) {
        var zoom = getCurrentZoom();
        var nearZoom = roundValueByArrayOfSteps(zoom, zoomValues, offset < 0);
        var nextZoomOffset = offset;
        if (zoom !== nearZoom) {
            nextZoomOffset += nextZoomOffset > 0 ? -1 : 1;
        }
        var currentZoomIndex = zoomValues.findIndex(function (value) { return value === nearZoom; }) || 0;
        var nextZoomIndex = currentZoomIndex + nextZoomOffset;
        var nextZoom = zoom;
    ...
    var zoomPage = $axure.player.zoomPage = function (zoom, position) {
        ...
        var newPageX = 0;
        var newPageY = 0;
        if (containerSize.width <= (pageSize.width * nextZoom) / currentZoom ||
            containerSize.height <= (pageSize.height * nextZoom) / currentZoom) {
            var mouseX = position ? position.x : containerSize.width / 2;
            var mouseY = position ? position.y : containerSize.height / 2;
            // get current page position
            var pagePosition = getPagePosition();
            // calculate new page position after zoom
            var mousePageX = mouseX - containerSize.width / 2 - pagePosition.x;
            var mousePageY = mouseY - containerSize.height / 2 - pagePosition.y;
            var newMousePageX = (mousePageX * nextZoom) / currentZoom;
            var newMousePageY = (mousePageY * nextZoom) / currentZoom;
    ...
    function normalizeZoomWheelEvent(e) {
        var DOM_DELTA_PIXELS = 0;
        var DOM_DELTA_LINES = 1;
        var DOM_DELTA_PAGES = 2;
        ...
        if (isMac()) {
            ...
            var lowResMouse = false;
            var lowResCutoff = 120 - 1;
    ...
    var startDrag = function (event) {
        ...
        function move(_a) {
            ...
            if (clientX === undefined || clientY === undefined)
                return;
            var changeX = clientX - mouseX;
            var changeY = clientY - mouseY;
            var pagePosition = getPageScrollPosition();
    ...
    function calculateClippingBoundsWidth(panelSize, isLeftPanel) {
        ...
        if (typeof panelSize !== 'undefined') {
            if (isLeftPanel)
                leftPanelOffset = panelSize;
            else
                rightPanelOffset = panelSize;
        }
    ...
    function calculateClippingBoundsScrollPosition() {
        ...
        if (isCentered && scaleVal == 1)
            leftPos = 0;
        else if (isCentered && scaleVal == 2)
            leftPos = $('#mainPanelContainer').width() * scale / 2.0 - contentLeftOfOriginOffset;
    ...
    $axure.player.collapseToBar = function (context, hostId) {
        ...
        if (context === 'project' || context === 'all') {
            if (!isMobileMode()) {
                ...
                $('.leftPanel').animate({ 'margin-left': -newWidth + 'px' }, { duration: 200, complete: function () { $('.leftPanel').width(0).hide().css({ 'marginLeft': '' }); } });
                $('#lsplitbar').animate({ left: '-4px' }, { duration: 200, complete: function () { $('#lsplitbar').hide(); } });
                $('#clippingBounds').animate({ left: '', width: clippingWidth + 'px' }, { duration: 200 });
                $('#clippingBoundsScrollContainer').animate({ left: newLeft + 'px' }, { duration: 200, complete: function () {
                        isAnimating = false;
                        $axure.player.resizeContent();
        ...
        if (context === 'inspect' || context === 'all') {
            if (!isMobileMode()) {
                ...
                $('.rightPanel').animate({ 'margin-right': -newWidth + 'px' }, { duration: 200, complete: function () { $('.rightPanel').width(0).hide().css({ 'marginRight': '' }); } });
                $('#rsplitbar').animate({ left: $(window).width() + 'px' }, { duration: 200, complete: function () { $('#rsplitbar').hide(); } });
    ...
    $axure.player.refreshViewPort = function () {
        ...
        if (!h || !clipToView)
            h = mainPanelHeight;
        if (MOBILE_DEVICE && h > mainPanelHeight)
            h = mainPanelHeight;
        if (MOBILE_DEVICE && w > mainPanelWidth)
            w = mainPanelWidth;
        var mainFrameHeight = h;
        var mainFrameWidth = w;
        if (clipToView) {
            if (!MOBILE_DEVICE && scaleVal == '0')
                scaleVal = 2;
            w = Number(w);
            h = Number(h);
            $('#mainFrame').width(w);
    ...
    function setUserLoggedInStatus(response, safariAuthResponseProfile) {
        if (!response.success) {
            ...
            if (safariAuthResponseProfile)
                response = safariAuthResponseProfile;
        ...
        if (typeof feedback !== 'undefined')
            feedback.updateUserAccountInfo(userAcct, authCookieValue, userCookieValue);
    }
    ...
    */
    ...
    function appendMobileBrowserControlFrame() {
        ...
        if (IOS)
            $('#mobileControlFrameContainer').on($axure.eventNames.mouseDownName, function (e) { e.stopPropagation(); });
    }
    ...
    function showPopup($container) {
        if ($('#browserOutOfDateNotification').is(":visible"))
            return;
    ...
    function repositionPinsOnScaleChange(data) {
        ...
        for (var i = 0; i < $pins.length; i++) {
            ...
            var left = parseFloat($($pins[i]).attr('data-x'));
            var top_1 = parseFloat($($pins[i]).attr('data-y'));
            var width = $($pins[i]).width();
            var height = $($pins[i]).height();
            // Get current scale of mainPanelContainer
            // MainPanelContainer scaled without setContentScale message
            var scale = $('#mainPanelContainer').css('transform');
            scale = (scale == "none") ? 1 : Number(scale.substring(scale.indexOf('(') + 1, scale.indexOf(',')));
            var scaledLeft = (left * scale) - (width / 2);
            var scaledTop = (top_1 * scale) - (height / 2);
    ...
    function messageCenter_message(message, data) {
        if (message == 'expandFrame')
            expand();
        else if (message == 'getCollapseFrameOnLoad' && $axure.player.settings.startCollapsed && !MOBILE_DEVICE)
            $axure.messageCenter.postMessage('collapseFrameOnLoad');
    ...
    function loadVariablesFromUrl(removeVarFromUrl) {
        var originalHashValues = window.location.href.substr(window.location.href.indexOf('#')) || '';
        var variables = {};
        var query = (originalHashValues.split(GLOBAL_VAR_NAME)[1] || '');
        if (query.length > 0) {
    ...
    var getPageUrlsById = $axure.player.getPageUrlsById = function (packageId, foundById, nodes) {
        if (!nodes)
            nodes = $axure.document.sitemap.rootNodes;
        for (var i = 0; i < nodes.length; i++) {
            var node = nodes[i];
            if (node.id == packageId)
                foundById.push(node.url);
    ...
    function deleteHashStringVar(currentHash, varName) {
        ...
        if (pageIndex == -1)
            pageIndex = currentHash.indexOf('&' + varWithEqual);
        if (pageIndex != -1) {
    ...
    function setUpController() {
        ...
        _player.createPluginHost = function (settings) {
            ...
            if (!settings.id)
                throw ('each plugin host needs an id');
            if (typeof PREVIEW_INFO === 'undefined') {
            ...
            if (!_player.isMobileMode())
                host.hide();
            else
                _player.updatePlugins();
...
//# sourceMappingURL=axplayer.js.map