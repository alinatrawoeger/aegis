...
(function () {
    ...
    $(window.document).ready(function () {
        ...
        $(window.document).bind('keyup', function (e) {
            if (e.target.localName == "textarea" || e.target.localName == "input" || event.target.isContentEditable)
                return;
            switch (e.which) {
                case 188:
                    openPreviousPage();
        ...
        $axure.page.bind('load.sitemap', function () {
            ...
            if (viewStr.length > 0) {
                ...
                if ($view.length > 0)
                    ...
                    $('.adaptiveViewOption[val="auto"]').click();
            }
    ...
    function generateAdaptiveViews(forProjectOptions) {
        ...
        if (!hasViews) {
            if (forProjectOptions) {
                ...
                $('#interfaceAdaptiveViewsContainer').hide();
        }
        else {
            if (forProjectOptions) {
                ...
                $('#interfaceAdaptiveViewsContainer').show();
        }
    ...
    function vpScaleOption_click(event) {
        ...
        if (scaleCheckDiv.hasClass('selectedRadioButton'))
            return false;
